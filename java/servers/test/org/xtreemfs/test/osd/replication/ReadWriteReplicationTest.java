...
public class ReadWriteReplicationTest {
	...
	private void testReplicatedWrite(int stripeSize, int fileSize) throws Exception {
		...
		client.createVolume(
				...
				StripingPolicyType.STRIPING_POLICY_RAID0, stripeSize, 1,
				new ArrayList<KeyValuePair>());
		
		AdminVolume volume = client.openVolume(volumeName, null, options);
		
		// sequential so we can control 
		volume.setDefaultReplicationPolicy(
				userCredentials,
				"/",
				ReplicaUpdatePolicies.REPL_UPDATE_PC_WQRQ, 3, ReplicationFlags.setSequentialStrategy(0));
		
		AdminFileHandle fileHandle = volume.openFile(
				userCredentials,
				path,
				SYSTEM_V_FCNTL.SYSTEM_V_FCNTL_H_O_CREAT_VALUE |
				SYSTEM_V_FCNTL.SYSTEM_V_FCNTL_H_O_TRUNC_VALUE |
				SYSTEM_V_FCNTL.SYSTEM_V_FCNTL_H_O_RDWR_VALUE,
				0777);
		...
		fileHandle = volume.openFile(
				userCredentials,
				path,
				SYSTEM_V_FCNTL.SYSTEM_V_FCNTL_H_O_RDONLY_VALUE,
				0777);
	...
	private void readMajority(
			List<Replica> replicas, FileHandle fileHandle,
			Object me,
			byte[] expected,
			int[] startList, int[] stopList) throws Exception {
		if(stopList != null) {
			for(int i : stopList) {
				Logging.logMessage(Logging.LEVEL_NOTICE, me, "Stopping OSD %d with UUID %s", i, replicas.get(i).getOsdUuids(0));
		...
		if(startList != null) {
			for(int i : startList) {
				Logging.logMessage(Logging.LEVEL_NOTICE, me, "Starting OSD %d with UUID %s", i, replicas.get(i).getOsdUuids(0));
