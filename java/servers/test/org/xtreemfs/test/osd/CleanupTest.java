...
public class CleanupTest extends TestCase {
    ...
    public CleanupTest() {
        ...
        passwd = Auth.newBuilder().setAuthType(AuthType.AUTH_PASSWORD)
                .setAuthPasswd(AuthPassword.newBuilder().setPassword("foo")).build();
    }
    ...
    private List<String> makeCleanup(boolean restore, boolean deleteVolumes, boolean killZombies)
        ...
        // start the cleanUp Operation
        r = env.getOSDClient().xtreemfs_cleanup_start(env.getOSDAddress(), passwd,
                RPCAuthentication.userService, killZombies, deleteVolumes, restore);
        r.get();
        r.freeBuffers();

        boolean isRunning = true;
        do {
            r = env.getOSDClient().xtreemfs_cleanup_status(env.getOSDAddress(), passwd,
            ...
            r = env.getOSDClient().xtreemfs_cleanup_is_running(env.getOSDAddress(), passwd,
                    RPCAuthentication.userService);
            isRunning = ((xtreemfs_cleanup_is_runningResponse) r.get()).getIsRunning();
            r.freeBuffers();
        } while (isRunning);

        r = env.getOSDClient().xtreemfs_cleanup_get_results(env.getOSDAddress(), passwd,
                RPCAuthentication.userService);
    ...
    private void setupTestVolume() throws Exception {
        ...
        c.start();
        c.createVolume("test", RPCAuthentication.authNone, uc, sp,
                AccessControlPolicyType.ACCESS_CONTROL_POLICY_POSIX, 511);
