...
public class SetupUtils {
    ...
    private static Properties createOSDProperties(int port, String dir) {
        ...
        props.setProperty(
                "debug.categories",
                ""
                        + Arrays.toString(DEBUG_CATEGORIES).substring(1,
                                Arrays.toString(DEBUG_CATEGORIES).length() - 1));
        ...
        props.setProperty("admin_password", "foo");
        return props;
    }
    ...
    public static StripingPolicy getStripingPolicy(int width, int stripeSize) {
        return StripingPolicy.newBuilder().setType(StripingPolicyType.STRIPING_POLICY_RAID0)
                .setStripeSize(stripeSize).setWidth(width).build();
    }
