...
public class VolumeTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		mrcAddress = testEnv.getMRCAddress().getHostName() + ":"
				+ testEnv.getMRCAddress().getPort();

		defaultCoordinates = VivaldiCoordinates.newBuilder().setXCoordinate(0)
				.setYCoordinate(0).setLocalError(0).build();
		defaultStripingPolicy = StripingPolicy.newBuilder()
	...
	public void testStatVFS() throws Exception, VolumeNotFoundException {
		...
		Client client = Client.createClient(dirAddress, userCredentials, null,
				options);
		client.start();
		...
		MRCServiceClient mrcClient = new MRCServiceClient(
				testEnv.getRpcClient(), null);

		StatVFS mrcClientVFS = null;
		RPCResponse<StatVFS> resp = null;
		try {
			statvfsRequest input = statvfsRequest.newBuilder()
					.setVolumeName(VOLUME_NAME_1).setKnownEtag(0).build();
			resp = mrcClient.statvfs(testEnv.getMRCAddress(), auth,
	...
	public void testCreateDelete() throws Exception {
		...
		// create volume
		client.createVolume(mrcAddress, auth, userCredentials, VOLUME_NAME, 0,
		...
		for (int i = 0; i < fileHandles.length; i++) {
			fileHandles[i] = volume.openFile(userCredentials, DIR1 + "/"
					+ TESTFILE + i,
		...
		try {
			volume.openFile(
					...
					SYSTEM_V_FCNTL.SYSTEM_V_FCNTL_H_O_EXCL.getNumber()
							| SYSTEM_V_FCNTL.SYSTEM_V_FCNTL_H_O_CREAT
									.getNumber());
		...
		try {
			volume.openFile(userCredentials, DIR1 + "/" + TESTFILE
					+ "1/foo.txt",
					SYSTEM_V_FCNTL.SYSTEM_V_FCNTL_H_O_CREAT.getNumber());
	...
	public void testHardLink() throws Exception {
		...
		try {
			resp = mrcClient.open(testEnv.getMRCAddress(), auth,
					userCredentials, VOLUME_NAME, ORIG_FILE,
		...
		try {
			resp = mrcClient.open(testEnv.getMRCAddress(), auth,
					userCredentials, VOLUME_NAME, "test-hardlink.txt",
					FileAccessManager.O_CREAT, 0, 0, defaultCoordinates);
			open = resp.get();
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			resp1 = mrcClient.getattr(testEnv.getMRCAddress(), auth,
					userCredentials, VOLUME_NAME, ORIG_FILE, 0);
			stat1 = resp1.get().getStbuf();

			resp2 = mrcClient.getattr(testEnv.getMRCAddress(), auth,
					userCredentials, VOLUME_NAME, LINKED_FILE, 0);
		...
		try {
			...
			resp2 = mrcClient.getattr(testEnv.getMRCAddress(), auth,
					userCredentials, VOLUME_NAME, LINKED_FILE2, 0);
			stat2 = resp2.get().getStbuf();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
		...
		try {
			mrcClient.getattr(testEnv.getMRCAddress(), auth, userCredentials,
					VOLUME_NAME, LINKED_FILE2, 0).get();
	...
	public void testGetSetAttr() throws Exception {
		...
		stat = stat.toBuilder().setGroupId("foobar").setUserId("FredFoobar")
				.build();
		try {
			volume.setAttr(
					userCredentials,
					TESTDIR,
					stat,
					Setattrs.SETATTR_UID.getNumber()
							| Setattrs.SETATTR_GID.getNumber());
			fail("changing username and groups should be restricted to superuser");
		} catch (PosixErrorException e) {
		}

		UserCredentials rootCreds = userCredentials.toBuilder()
				.setUsername("root").setGroups(0, "root").build();
		...
		volume.setAttr(userCredentials, TESTDIR + "/" + TESTFILE, stat,
				Setattrs.SETATTR_MODE.getNumber());
		stat = volume.getAttr(userCredentials, TESTDIR + "/" + TESTFILE);
	...
	public void testReplicaAddListRemove() throws Exception {
		...
		volume.setXAttr(userCredentials, TESTFILE, "xtreemfs."
				+ SysAttrs.set_repl_update_policy.toString(),
				ReplicaUpdatePolicies.REPL_UPDATE_PC_WQRQ,
				XATTR_FLAGS.XATTR_FLAGS_CREATE.getNumber());

		// start a second OSD. This ensures the file is not assigned to this
		// OSD.
		...
		Replica replica = Replica.newBuilder()
				.addOsdUuids(configs[1].getUUID().toString())
	...
	public void testSetGetListXattr() throws Exception {
		...
		volume.openFile(userCredentials, TESTFILE,
				SYSTEM_V_FCNTL.SYSTEM_V_FCNTL_H_O_CREAT.getNumber());

		int initialNumberOfXattr = volume.listXAttrs(userCredentials, TESTFILE)
				.getXattrsCount();
		...
		assertEquals(-1,
				volume.getXAttrSize(userCredentials, TESTFILE, "foobarasdf"));

		// creating a new Xattr should increase the number of xattrs
		volume.setXAttr(userCredentials, TESTFILE, "foobarasdf", "nyancat",
				XATTR_FLAGS.XATTR_FLAGS_CREATE.getNumber());

		assertEquals(initialNumberOfXattr + 1,
				volume.listXAttrs(userCredentials, TESTFILE).getXattrsCount());
		...
		assertEquals(initialNumberOfXattr,
				volume.listXAttrs(userCredentials, TESTFILE).getXattrsCount());
		xattr = volume.getXAttr(userCredentials, TESTFILE, "foobarasdf");
		assertNull(xattr);
		...
		xattr = volume.getXAttr(userCredentials, TESTFILE, "xtreemfs."
				+ SysAttrs.set_repl_update_policy.toString());
		assertEquals(ReplicaUpdatePolicies.REPL_UPDATE_PC_NONE, xattr);

		// read existing systemflag
		xattr = volume.getXAttr(userCredentials, TESTFILE, "xtreemfs."
				+ SysAttrs.object_type.toString());
