...
public class MRCTest extends TestCase {
    ...
    public void testCreateDeleteListVolumes() throws Exception {
        ...
        for (int i = 0; i < numVols; i++) {
            ...
            invokeSync(client.xtreemfs_mkvol(mrcAddress, RPCAuthentication.authNone, uc,
                    AccessControlPolicyType.ACCESS_CONTROL_POLICY_POSIX, getDefaultStripingPolicy(), "",
                    0775, name, "", "", getKVList("bla", "blub")));
    ...
    public void testCreateDelete() throws Exception {
        ...
        assertTrue("test2.txt is a not a file",
                (stat.getMode() & SYSTEM_V_FCNTL.SYSTEM_V_FCNTL_H_S_IFREG.getNumber()) != 0);
    ...
    public void testXAttrs() throws Exception {
        ...
        try {
            val = invokeSync(
                    client.getxattr(mrcAddress, RPCAuthentication.authNone, uc, volumeName, "test2.txt",
                            "key1")).getValue();
    ...
    public void testOpenCreateNoPerm() throws Exception {
        ...
        try {
            invokeSync(client
                    .open(mrcAddress, RPCAuthentication.authNone, uc2, volumeName, "test2.txt",
    ...
    public void testAccessControl() throws Exception {
        ...
        Stat stat = invokeSync(
                client.getattr(mrcAddress, RPCAuthentication.authNone, uc3, noACVolumeName, "chownTestFile",
                        -1)).getStbuf();
    ...
    public void testFileSizeUpdate() throws Exception {
        ...
        XCap cap = invokeSync(
                client.open(mrcAddress, RPCAuthentication.authNone, uc, volumeName, fileName,
                        FileAccessManager.O_RDONLY, 0, 0, getDefaultCoordinates())).getCreds().getXcap();
        Stat stat = invokeSync(
                client.getattr(mrcAddress, RPCAuthentication.authNone, uc, volumeName, fileName, -1))
                .getStbuf();
        ...
        invokeSync(client.xtreemfs_update_file_size(mrcAddress, RPCAuthentication.authNone,
                RPCAuthentication.userService, xtreemfs_update_file_sizeRequest.newBuilder()
                        .setOsdWriteResponse(owr).setXcap(cap).build()));
        stat = invokeSync(
                client.getattr(mrcAddress, RPCAuthentication.authNone, uc, volumeName, fileName, -1))
                .getStbuf();
    ...
    public void testDefaultStripingPolicies() throws Exception {
        ...
        invokeSync(client.xtreemfs_mkvol(mrcAddress, RPCAuthentication.authNone, uc,
                AccessControlPolicyType.ACCESS_CONTROL_POLICY_NULL, sp1, "", 0, volumeName, "", "",
                getKVList()));
        ...
        Stat stat = invokeSync(
                client.getattr(mrcAddress, RPCAuthentication.authNone, uc, volumeName, fileName1, -1))
                .getStbuf();
    ...
    public void testDefaultReplicationPolicies() throws Exception {
        ...
        final StripingPolicy sp = StripingPolicy.newBuilder()
                .setType(StripingPolicyType.STRIPING_POLICY_RAID0).setStripeSize(64).setWidth(1).build();
        ...
        // create a new file in a directory in a new volume
        invokeSync(client.xtreemfs_mkvol(mrcAddress, RPCAuthentication.authNone, uc,
                AccessControlPolicyType.ACCESS_CONTROL_POLICY_NULL, sp, "", 0, volumeName, "", "",
                getKVList()));

        invokeSync(client.setxattr(mrcAddress, RPCAuthentication.authNone, uc, volumeName, "",
                "xtreemfs.default_rp", "",
                ByteString.copyFrom(Converter.replicationPolicyToJSONString(rp).getBytes()), 0));

        String val = invokeSync(
                client.getxattr(mrcAddress, RPCAuthentication.authNone, uc, volumeName, "",
    ...
    public void testReplicateOnClose() throws Exception {
        ...
        assertTrue((ReplicationFlags.setReplicaIsComplete(0) & xLoc.getReplicas(0).getReplicationFlags()) != 0);
