...
public class SnapshotTest extends TestCase {
    ...
    public void testSnapshots() throws Exception {
        ...
        for (TreeEntry path : tree)
            if (path.isDir())
                invokeSync(client.mkdir(mrcAddress, RPCAuthentication.authNone, uc, volumeName, path
                        .getName(), 0775));
            else
        ...
        for (TreeEntry path : entries)
            if (path.isDir())
                invokeSync(client.rmdir(mrcAddress, RPCAuthentication.authNone, uc, volumeName, path
                        .getName()));
            else
                invokeSync(client.unlink(mrcAddress, RPCAuthentication.authNone, uc, volumeName, path
                        .getName()));
