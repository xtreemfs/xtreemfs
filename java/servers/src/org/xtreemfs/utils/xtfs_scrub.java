...
public class xtfs_scrub {
                        ...
                        if (repair) {
                            ...
                            if (numNewReplicas == numFullReplRm) {
                                printFileErrorMessage(file.getFile(), file.getFileId(), "lost "
                                        + numFullReplRm + " replicas due to dead OSDs. Created "
                                        + numFullReplRm + " new replicas.");
                            } else {
                                printFileErrorMessage(file.getFile(), file.getFileId(), "lost "
    ...
    public xtfs_scrub(Client client, String volume, int numThrs, boolean repair, boolean delete,
        ...
        /*
         * this.rpcClient = rpcClient; this.dirClient = dirClient; this.mrcClient = mrcClient;
         */
                ...
                while (numInFlight < numThrs) {
                    try {
                        ...
                        try {
                            ...
                            RandomAccessFile file = f.canWrite() ? f.open("rw", 0) : f.open("r", 0);
