...
public class PreprocStage extends Stage {
    ...
    private void doPrepareRequest(StageRequest rq) {
        ...
        if (request.getOperation().requiresCapability()) {
            ...
            if (err != null) {
                ...
                if (Logging.isDebug()) {
                    Logging.logMessage(Logging.LEVEL_DEBUG, Category.net, this, "authentication of request failed: %s",
                            ErrorUtils.formatError(err));
                }
                return;
            }
        }
    ...
    public void acquireLock(String clientUuid, int pid, String fileId, long offset, long length, boolean exclusive,
        ...
        this.enqueueOperation(STAGEOP_ACQUIRE_LOCK,
                new Object[] { clientUuid, pid, fileId, offset, length, exclusive }, request, listener);
    }
    ...
    public void doAcquireLock(StageRequest m) {
        ...
        try {
            ...
            if (e == null) {
                callback.parseComplete(null, ErrorUtils.getErrorResponse(ErrorType.INTERNAL_SERVER_ERROR,
            ...
            if (l != null) {
                Lock lock = Lock.newBuilder().setClientPid(l.getClientPid()).setClientUuid(l.getClientUuid())
                        .setLength(l.getLength()).setOffset(l.getOffset()).setExclusive(l.isExclusive()).build();
                callback.parseComplete(lock, null);
            } else
                callback.parseComplete(null,
                        ErrorUtils.getErrorResponse(ErrorType.ERRNO, POSIXErrno.POSIX_ERROR_EAGAIN, "conflicting lock"));
    ...
    public void checkLock(String clientUuid, int pid, String fileId, long offset, long length, boolean exclusive,
            OSDRequest request, LockOperationCompleteCallback listener) {
        this.enqueueOperation(STAGEOP_CHECK_LOCK, new Object[] { clientUuid, pid, fileId, offset, length, exclusive },
                request, listener);
    }
            ...
            for (OpenFileTable.OpenFileTableEntry entry : closedFiles) {
                ...
                if (Logging.isDebug())
                    Logging.logMessage(Logging.LEVEL_DEBUG, Category.stage, this,
                            "send internal close event for %s, deleteOnClose=%b", entry.getFileId(),
                            entry.isDeleteOnClose());
            ...
            if (rqPrototype == null) {
                ...
                if (rq.getRPCRequest().getMessage() != null) {
                    rq.setRequestArgs(rqPrototype.newBuilderForType()
                            .mergeFrom(new ReusableBufferInputStream(rq.getRPCRequest().getMessage())).build());
                    if (Logging.isDebug()) {
                        Logging.logMessage(Logging.LEVEL_DEBUG, Category.net, this, "received request of type %s", rq
                                .getRequestArgs().getClass().getName());
                    }
                } else {
                    rq.setRequestArgs(rqPrototype.getDefaultInstanceForType());
                    if (Logging.isDebug()) {
                        Logging.logMessage(Logging.LEVEL_DEBUG, Category.net, this,
                                "received request of type %s (empty message)", rq.getRequestArgs().getClass().getName());
                    }
                }
            }
