...
public class RWReplicationStage extends Stage implements FleaseMessageSenderInterface {
    ...
    public static enum Operation {
        READ, WRITE, TRUNCATE, INTERNAL_UPDATE, INTERNAL_TRUNCATE
    };
    ...
    public RWReplicationStage(OSDRequestDispatcher master, SSLOptions sslOpts) throws IOException {
        ...
        FleaseConfig fcfg = new FleaseConfig(master.getConfig().getFleaseLeaseToMS(), master.getConfig()
                .getFleaseDmaxMS(), master.getConfig().getFleaseDmaxMS(), null, localID.toString(), master.getConfig()
                .getFleaseRetries());
    ...
    private void executeSetAuthState(final ReplicaStatus localState, final AuthoritativeReplicaState authState,
        ...
        Map<Long, ObjectVersionInfo> objectsToBeDeleted = new HashMap();
        for (ObjectVersion localObject : localState.getObjectVersionsList()) {
            ...
            if (localObject.getObjectVersion() <= authState.getMaxObjVersion()) {
                objectsToBeDeleted.put(localObject.getObjectNumber(),
                        new ObjectVersionInfo(localObject.getObjectVersion(), 0, 0));
            }
        }
        ...
        for (ObjectVersionMapping authObject : authState.getObjectVersionsList()) {
            ObjectVersionInfo version = objectsToBeDeleted.get(authObject.getObjectNumber());
            if ((version != null) && (version.version == authObject.getObjectVersion())) {
        ...
        if (!missingObjects.isEmpty() || !objectsToBeDeleted.isEmpty()
            ...
            // TODO: properly handle versions w/ different timestamps, which are created when snapshots / COW
            // is enabled
        ...
        try {
            ...
            if (fileId != null) {
                ...
                if (error == null) {
                    ...
                    if (oldState == ReplicaState.PRIMARY && lease.getLeaseHolder() == null
                ...
                public void responseAvailable(RPCResponse r) {
                    try {
                        ...
                        eventObjectFetched(
                                ...
                                record,
                                null,
                                ErrorUtils.getErrorResponse(ErrorType.IO_ERROR, POSIXErrno.POSIX_ERROR_NONE,
                                        ex.toString(), ex));
                    } finally {
                        r.freeBuffers();
                    }
    ...
    private void doBackup(final ReplicatedFileState file) {
        ...
        /*
         * } catch (IOException ex) { failed(file, ErrorUtils.getErrorResponse(ErrorType.ERRNO,
     ...
     * RWReplicationCallback callback, OSDRequest request) { this.enqueueOperation(STAGEOP_OPEN, new
     * Object[]{credentials,locations,forceReset}, request, callback); }
     */
    ...
    protected void processMethod(StageRequest method) {
        ...
        case STAGEOP_CLOSE:
            processFileClosed(method);
            break;
        case STAGEOP_PROCESS_FLEASE_MSG:
            processFleaseMessage(method);
            break;
        ...
        case STAGEOP_INTERNAL_AUTHSTATE:
            processSetAuthoritativeState(method);
            break;
        case STAGEOP_LEASE_STATE_CHANGED:
            processLeaseStateChanged(method);
            break;
        case STAGEOP_INTERNAL_OBJFETCHED:
            processObjectFetched(method);
            break;
        case STAGEOP_INTERNAL_STATEAVAIL:
            processReplicaStateAvailExecReset(method);
            break;
        case STAGEOP_INTERNAL_DELETE_COMPLETE:
            processDeleteObjectsComplete(method);
            break;
        case STAGEOP_INTERNAL_MAXOBJ_AVAIL:
            processMaxObjAvail(method);
            break;
        case STAGEOP_INTERNAL_BACKUP_AUTHSTATE:
            processBackupAuthoritativeState(method);
            break;
        case STAGEOP_FORCE_RESET:
            processForceReset(method);
            break;
        case STAGEOP_GETSTATUS:
            processGetStatus(method);
            break;
        default:
    ...
    private void processGetStatus(StageRequest method) {
        ...
        try {
            ...
            for (String fileId : this.files.keySet()) {
                ...
                fStatus.put("pending requests", fState.getPendingRequests() == null ? "0" : ""
                        + fState.getPendingRequests().size());
