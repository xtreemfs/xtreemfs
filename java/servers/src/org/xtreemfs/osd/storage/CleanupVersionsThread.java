            ...
            do {
                ...
                do {
                    ...
                    for (String fileName : l.files.keySet()) {
                        ...
                        if (flist == null) {
                            ...
                            ServiceSet s = master.getDIRClient().xtreemfs_service_get_by_uuid(null,
                                    RPCAuthentication.authNone, RPCAuthentication.userService, vol.id);
                            ...
                            vol.mrc = new ServiceUUID(KeyValuePairs.getValue(s.getServices(0).getData().getDataList(),
                                    "mrc"));
                            ...
                            try {
                                ...
                                tsResponse = mrcClient.getxattr(
                                        vol.mrc.getAddress(),
                                        RPCAuthentication.authNone,
                                        uc,
                                        getxattrRequest.newBuilder().setVolumeName(volName).setPath("")
                                                .setName("xtreemfs.snapshots").build());
                                ts = tsResponse.get().getValue();

                                // parse the list
                                StringTokenizer st = new StringTokenizer(ts);
                                String[] snapArray = new String[st.countTokens()];
                                for (int i = 0; i < snapArray.length; i++)
                                    snapArray[i] = st.nextToken();

                                vol.timestamps = new long[snapArray.length];

                                // retrieve the snapshot timestamp for each snapshot
                                for (int i = 0; i < vol.timestamps.length; i++) {

                                    tsResponse = mrcClient.getxattr(vol.mrc.getAddress(), RPCAuthentication.authNone,
                                            uc,
                                            getxattrRequest.newBuilder().setVolumeName(volName + "@" + snapArray[i])
                                                    .setPath("").setName("xtreemfs.snapshot_time").build());
                                    ts = tsResponse.get().getValue();
                                    vol.timestamps[i] = Long.parseLong(ts);
                                }
                ...
                for (Entry<Volume, List<String>> entry : perVolume.entrySet()) {
                    ...
                    for (String fileId : files) {
                        ...
                        // (doesn't matter here ...)
                        FileMetadata md = layout.getFileMetadataNoCaching(
                                RAID0Impl.getPolicy(
                                        Replica.newBuilder()
                                                ...
                                                .setStripingPolicy(
                                                        StripingPolicy.newBuilder()
                                                                ...
                                                                .setStripeSize(128).setWidth(1).build()).build(), 0),
                                fileId);

                        // remove all unbound file versions from the file version log
                        long[] remainingTimestamps = md.getVersionManager().purgeUnboundFileVersions(timestamps);

                        // determine the set of object versions that are not bound to any file version and can
                        // thus be safely deleted
                        Map<Long, Set<ObjectVersionInfo>> unboundObjectVersions = md.getVersionManager()
                                .getUnboundObjectVersions(remainingTimestamps);

                        // delete the objects
                        for (Entry<Long, Set<ObjectVersionInfo>> v : unboundObjectVersions.entrySet())
                            for (ObjectVersionInfo version : v.getValue()) {
                                // delete the object version
                                layout.deleteObject(fileId, md, v.getKey(), version.version, version.timestamp);
                            }
