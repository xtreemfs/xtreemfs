...
public class SingleFileStorageLayout extends StorageLayout {
    ...
    protected FileMetadata loadFileMetadata(String fileId, StripingPolicyImpl sp) throws IOException {
        ...
        if (f.exists()) {
            ...
            for (long i = 0; i < numObjs; i++) {
                ...
                if (version == 0)
                    continue;
                fi.getVersionManager().addObjectVersionInfo(globalON, version, 0, chkSum);
            }
    ...
    public void writeObject(String fileId, FileMetadata md, ReusableBuffer data, long objNo, int offset,
        ...
        md.getVersionManager().addObjectVersionInfo(objNo, newVersion, 0, newChecksum);
    }

    private void writeMDRecord(FileMetadata md, RandomAccessFile mdfile, long objNo, long version, long checksum)
            throws IOException {
