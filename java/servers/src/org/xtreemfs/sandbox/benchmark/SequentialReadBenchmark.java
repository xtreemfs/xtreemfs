...
class SequentialReadBenchmark extends SequentialBenchmark {
    ...
    long performIO(byte[] data, long numberOfBlocks) throws IOException {
        FileHandle fileHandle = volume.openFile(config.userCredentials, filenames[0],
        ...
        for (long j = 0; j < numberOfBlocks; j++) {
            ...
            byteCounter += fileHandle.read(config.userCredentials, data, stripeWidth, nextOffset);
        }
        fileHandle.close();
        return byteCounter;
    }
