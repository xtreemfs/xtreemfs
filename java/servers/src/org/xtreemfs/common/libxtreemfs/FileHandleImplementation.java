...
public class FileHandleImplementation implements FileHandle {
	...
	public int read(UserCredentials userCredentials, ReusableBuffer buf,
		...
		StripingPolicy policy = fc.getXlocs().getReplicas(0)
				.getStripingPolicy();
		...
		for (int j = 0; j < operations.size(); j++) {
			...
			ObjectData data = RPCCaller.<readRequest, ObjectData> syncCall(
					...
					volumeOptions, uuidResolver, uuidIterator, false,
	 ...
	 **/
	protected void truncatePhaseTwoAndThree(UserCredentials userCredentials,
			long newFileSize) throws IOException, PosixErrorException,
			AddressToUUIDNotFoundException {
	...
	protected void writeBackFileSizeAsync() throws IOException,
		...
		InetSocketAddress server = RPCCaller.getInetSocketAddressFromAddress(
				address, SERVICES.MRC);

		RPCResponse<timestampResponse> r = mrcServiceClient
				.xtreemfs_update_file_size(server, authBogus,
				...
				try {
					...
					if (Logging.isDebug()) {
						Logging.logMessage(
								Logging.LEVEL_DEBUG,
								this,
								"renewXcapAsync: The following "
										+ "error occured during the async all: ",
								e.getMessage());
					}
				} finally {
					r.freeBuffers();
	...
	protected void writeBackFileSize(OSDWriteResponse response,
			boolean closeFile) throws IOException, PosixErrorException,
			AddressToUUIDNotFoundException {
		if (Logging.isDebug()) {
			Logging.logMessage(
					Logging.LEVEL_DEBUG,
