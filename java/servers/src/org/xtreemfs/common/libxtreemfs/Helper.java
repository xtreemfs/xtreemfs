...
public class Helper {
	...
	public static String generateVersion4UUID() {
		...
		if (Logging.isDebug()) {
			Logging.logMessage(Logging.LEVEL_DEBUG, Category.misc,
					new Helper(), "Generated client UUID: %s", uuid);
		}
		return uuid;
	}
	...
	static public String getOSDUUIDFromXlocSet(XLocSet xlocs, int replicaIndex,
			int stripeIndex) {
		if (xlocs.getReplicasCount() == 0) {
			Logging.logMessage(
					...
					Category.misc,
					xlocs,
					"getOSDUUIDFromXlocSet: Empty replicas list in XlocSet: %s",
					xlocs.toString());
			return "";
		}
		...
		if (replica.getOsdUuidsCount() == 0) {
			Logging.logMessage(
					...
					Category.misc,
					xlocs,
					"GetOSDUUIDFromXlocSet: No head OSD available in XlocSet: %s",
					xlocs.toString());
			return "";
		}
	...
	/**
	 * Compares newResponse with currentResponse. Returns 0 iff newResponse ==
	 * currentResponse; 1 iff newResponse < currentResponse; -1 iff newResponse
	 * > currentResponse.
	 * 
	 * @param newResponse
	 * 
	...
	static protected int compareOSDWriteResponses(OSDWriteResponse newResponse,
		...
		if (newResponse.getTruncateEpoch() > currentResponse.getTruncateEpoch()
				|| (newResponse.getTruncateEpoch() == currentResponse
						.getTruncateEpoch() && newResponse.getSizeInBytes() > currentResponse
						.getSizeInBytes())) {
			return 1;
		}
	...
	/**
	 * Compares "lock1 "with "lock2". Returns true if lock1 equals lock2. false
	 * otherwise. They are equal iff clientUuid, clientPid, offset and length
	 * are equal.
	 * 
	 * @param lock1
	 * @param lock2
	...
	protected static boolean checkIfLocksAreEqual(Lock lock1, Lock lock2) {
		return lock1.getClientUuid().equals(lock2.getClientUuid())
				&& lock1.getClientPid() == lock2.getClientPid()
				&& lock1.getOffset() == lock2.getOffset()
	...
	protected static boolean checkIfLocksDoConflict(Lock lock1, Lock lock2) {
		...
		long lock1End = lock1.getLength() == 0 ? 0 : lock1.getLength()
				+ lock1.getOffset();
		long lock2End = lock2.getLength() == 0 ? 0 : lock2.getLength()
				+ lock2.getOffset();
