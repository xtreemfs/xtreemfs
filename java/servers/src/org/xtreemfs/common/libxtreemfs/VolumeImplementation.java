...
/**
 * This class represents the volume as it is used internally
 * by libxtreemfs-java.
 * 
 */
public class VolumeImplementation extends Volume {
	...
	/**
	 * Bogus object of UserCredentials. Used when no real UserCredentials are
	 * needed.
	 */
	...
	/**
	 * Maps a StripingPolicyType to a StripeTranslator. Should be filled with
	 * all possible StripingPolicys.
	 */
     ...
     * 
     */
	public VolumeImplementation(ClientImplementation client, String clientUUID,
			UUIDIterator mrcUuidIterator, String volumeName,
		...
		this.userCredentialsBogus = UserCredentials.newBuilder()
				.setUsername("xtreemfs").build();
		this.authBogus = RPCAuthentication.authNone;

		this.metadataCache = new MetadataCache(options.getMetadataCacheSize(),
				options.getMetadataCacheTTLs());
		// register all stripe translators
		this.stripeTranslators = new HashMap<StripingPolicyType, StripeTranslator>();
		stripeTranslators.put(StripingPolicyType.STRIPING_POLICY_RAID0,
				new StripeTranslatorRaid0());
	}
		...
		try {
			...
			if (Logging.isDebug()) {
				Logging.logMessage(Logging.LEVEL_DEBUG, Category.misc, this,
						"Volume: Couldn't shut"
								+ " down network client corretly: %s ",
						e.getMessage());
			}
		}
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#statFS(org.xtreemfs.foundation
	...
	public StatVFS statFS(UserCredentials userCredentials) throws IOException,
		...
		statvfsRequest request = statvfsRequest.newBuilder().setKnownEtag(0)
				.setVolumeName(volumeName).build();

		StatVFS stat = RPCCaller.<statvfsRequest, StatVFS> syncCall(
				SERVICES.MRC, userCredentials, authBogus, volumeOptions,
				uuidResolver, mrcUUIDIterator, false, request,
				new CallGenerator<statvfsRequest, StatVFS>() {
					@Override
					public RPCResponse<StatVFS> executeCall(
							InetSocketAddress server, Auth authHeader,
							UserCredentials userCreds, statvfsRequest input)
							throws IOException {
						return mrcServiceClient.statvfs(server, authHeader,
								userCreds, input);
					}
				});
		return stat;
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#readLink(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces.RPC .UserCredentials, java.lang.String,
	 * java.lang.String)
	 */
	@Override
	public String readLink(UserCredentials userCredentials, String path)
			throws IOException, PosixErrorException, AddressToUUIDNotFoundException, AddressToUUIDNotFoundException {
		readlinkRequest request = readlinkRequest.newBuilder()
				.setVolumeName(volumeName).setPath(path).build();

		readlinkResponse response = RPCCaller
				.<readlinkRequest, readlinkResponse> syncCall(SERVICES.MRC,
						userCredentials, authBogus, volumeOptions,
						uuidResolver, mrcUUIDIterator, false, request,
						new CallGenerator<readlinkRequest, readlinkResponse>() {
							@Override
							public RPCResponse<readlinkResponse> executeCall(
									InetSocketAddress server, Auth authHeader,
									UserCredentials userCreds,
									readlinkRequest input) throws IOException {
								return mrcServiceClient.readlink(server,
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#symlink(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces.RPC .UserCredentials, java.lang.String,
	 * java.lang.String)
	 */
	@Override
	public void symlink(UserCredentials userCredentials, String targetPath,
			String linkPath) throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		symlinkRequest request = symlinkRequest.newBuilder()
				.setLinkPath(linkPath).setTargetPath(targetPath)
				.setVolumeName(volumeName).build();

		timestampResponse response = RPCCaller
				.<symlinkRequest, timestampResponse> syncCall(SERVICES.MRC,
						userCredentials, authBogus, volumeOptions,
						uuidResolver, mrcUUIDIterator, false, request,
						new CallGenerator<symlinkRequest, timestampResponse>() {
							...
							public RPCResponse<timestampResponse> executeCall(
									InetSocketAddress server, Auth authHeader,
									UserCredentials userCreds,
									symlinkRequest input) throws IOException {
								return mrcServiceClient.symlink(server,
										authHeader, userCreds, input);
							}
						});
		...
		metadataCache.updateStatTime(
				parentDir,
				response.getTimestampS(),
				Setattrs.SETATTR_CTIME.getNumber()
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#link(org.xtreemfs.foundation.pbrpc
	 * .generatedinterfaces.RPC. UserCredentials, java.lang.String,
	 * java.lang.String)
	 */
	@Override
	public void link(UserCredentials userCredentials, String targetPath,
			String linkPath) throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		linkRequest request = linkRequest.newBuilder().setLinkPath(linkPath)
				.setTargetPath(targetPath).setVolumeName(volumeName).build();

		timestampResponse response = RPCCaller
				.<linkRequest, timestampResponse> syncCall(SERVICES.MRC,
						userCredentials, authBogus, volumeOptions,
						uuidResolver, mrcUUIDIterator, false, request,
						new CallGenerator<linkRequest, timestampResponse>() {
							...
							public RPCResponse<timestampResponse> executeCall(
									InetSocketAddress server, Auth authHeader,
									UserCredentials userCreds, linkRequest input)
									throws IOException {
								return mrcServiceClient.link(server,
										authHeader, userCreds, input);
							}
						});
		;
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#access(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces.RPC .UserCredentials, java.lang.String,
	 * org.xtreemfs.pbrpc.generatedinterfaces.MRC.ACCESS_FLAGS)
	 */
	@Override
	public void access(UserCredentials userCredentials, String path, int flags)
			throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		accessRequest request = accessRequest.newBuilder().setFlags(flags)
				.setPath(path).setVolumeName(volumeName).build();

		RPCCaller.<accessRequest, emptyResponse> syncCall(SERVICES.MRC,
				userCredentials, authBogus, volumeOptions, uuidResolver,
				mrcUUIDIterator, false, request,
				new CallGenerator<accessRequest, emptyResponse>() {
					@SuppressWarnings("unchecked")
					@Override
					public RPCResponse<emptyResponse> executeCall(
							InetSocketAddress server, Auth authHeader,
							UserCredentials userCreds, accessRequest input)
							throws IOException {
						return mrcServiceClient.access(server, authHeader,
								userCreds, input);
					}
				});
	}
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#openFile(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces.RPC .UserCredentials, java.lang.String,
	 * org.xtreemfs.pbrpc.generatedinterfaces.GlobalTypes.SYSTEM_V_FCNTL)
	 */
	@Override
	public FileHandle openFile(UserCredentials userCredentials, String path,
	...
	@Override
	public FileHandle openFile(UserCredentials userCredentials, String path,
			int flags, int mode) throws IOException, PosixErrorException,
	 ...
	 */
	public FileHandle openFile(UserCredentials userCredentials, String path,
			int flags, int mode, int truncateNewFileSize) throws IOException,
		...
		openRequest request = openRequest.newBuilder()
				.setVolumeName(volumeName).setPath(path).setFlags(flags)
				.setMode(mode).setAttributes(0).build();

		openResponse response = RPCCaller.<openRequest, openResponse> syncCall(
				SERVICES.MRC, userCredentials, authBogus, volumeOptions,
				uuidResolver, mrcUUIDIterator, false, request,
				new CallGenerator<openRequest, openResponse>() {
					@Override
					public RPCResponse<openResponse> executeCall(
							InetSocketAddress server, Auth authHeader,
							UserCredentials userCreds, openRequest input)
							throws IOException {
						return mrcServiceClient.open(server, authHeader,
								userCreds, input);
					}
				});
		...
		if (response.getCreds().getXlocs().getReplicasCount() == 0) {
			String errorMessage = "MRC assigned no OSDs to file on open" + path
					+ ", xloc: " + response.getCreds().getXlocs().toString();
		...
		FileInfo fileInfo = getOrCreateFileInfo(
				Helper.extractFileIdFromXcap(response.getCreds().getXcap()),
				path, response.getCreds().getXcap().getReplicateOnClose(),
				response.getCreds().getXlocs());

		fileHandle = fileInfo.createFileHandle(response.getCreds().getXcap(),
				asyncWritesEnabled);
		...
		if ((flags & SYSTEM_V_FCNTL.SYSTEM_V_FCNTL_H_O_TRUNC.getNumber()) > 0) {
			...
			if (Logging.isDebug()) {
				Logging.logMessage(Logging.LEVEL_DEBUG, Category.misc, this,
						"open called with O_TRUNK.");
			}

			try {
				fileHandle.truncatePhaseTwoAndThree(userCredentials,
						truncateNewFileSize);
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#truncate(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces.RPC .UserCredentials, java.lang.String, int)
	 */
	@Override
	public void truncate(UserCredentials userCredentials, String path,
			int newFileSize) throws IOException, PosixErrorException,
			AddressToUUIDNotFoundException {
		...
		FileHandle fileHandle = openFile(userCredentials, path, flags,
				newFileSize);
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#getAttr(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces.RPC .UserCredentials, java.lang.String,
	 * org.xtreemfs.pbrpc.generatedinterfaces.MRC.Stat)
	 */
	@Override
	public Stat getAttr(UserCredentials userCredentials, String path)
			throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
	 ...
	 */
	protected Stat getAttr(UserCredentials userCredentials, String path,
			FileInfo fileInfo) throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		...
		if (fileInfo == null) {
			...
			// fileId.
			FileInfo fileInfoFromOpenFileTable = openFileTable.get(stat
					.getIno()); // Ino == fileId
			if (fileInfoFromOpenFileTable != null) {
				...
				FileInfo fileInfoFromOpenFileTableAgain = openFileTable
						.get(stat.getIno()); // ino == fileID
				if (fileInfoFromOpenFileTableAgain != null) {
					stat = fileInfoFromOpenFileTableAgain
							.mergeStatAndOSDWriteResponse(stat);
				} else {
	...
	private Stat getAttrHelper(UserCredentials userCredentials, String path)
			throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		...
		if (stat != null) {
			...
			if (Logging.isDebug()) {
				Logging.logMessage(Logging.LEVEL_DEBUG, Category.misc, this,
						"getattr: serving from stat-cache %s  %s", path,
						stat.getSize());
			}
		} else {
			...
			getattrRequest request = getattrRequest.newBuilder()
					.setVolumeName(volumeName).setPath(path).setKnownEtag(0)
					.build();

			getattrResponse response = RPCCaller
					.<getattrRequest, getattrResponse> syncCall(
							SERVICES.MRC,
							userCredentials,
							authBogus,
							volumeOptions,
							...
							new CallGenerator<getattrRequest, getattrResponse>() {
								@Override
								public RPCResponse<getattrResponse> executeCall(
										InetSocketAddress server,
										Auth authHeader,
										UserCredentials userCreds,
										getattrRequest input)
										throws IOException {
									return mrcServiceClient.getattr(server,
											authHeader, userCreds, input);
								}
							});
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#setAttr(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces.RPC .UserCredentials, java.lang.String,
	 * org.xtreemfs.pbrpc.generatedinterfaces.MRC.Stat,
	 * org.xtreemfs.pbrpc.generatedinterfaces.MRC.Setattrs)
	 */
	@Override
	public void setAttr(UserCredentials userCredentials, String path,
			Stat stat, int toSet) throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		setattrRequest request = setattrRequest.newBuilder()
				.setVolumeName(volumeName).setPath(path).setStbuf(stat)
				.setToSet(toSet).build();

		timestampResponse response = RPCCaller
				.<setattrRequest, timestampResponse> syncCall(SERVICES.MRC,
						userCredentials, authBogus, volumeOptions,
						uuidResolver, mrcUUIDIterator, false, request,
						new CallGenerator<setattrRequest, timestampResponse>() {
							...
							public RPCResponse<timestampResponse> executeCall(
									InetSocketAddress server, Auth authHeader,
									UserCredentials userCreds,
									setattrRequest input) throws IOException {
								return mrcServiceClient.setattr(server,
										authHeader, userCreds, input);
							}
						});
		...
		if (((toSet & Setattrs.SETATTR_MODE.getNumber()) > 0)
			...
			stat = stat.toBuilder()
					.setAtimeNs(1000000000L * response.getTimestampS()).build();
		}
		...
		if (stat.getNlink() > 1
				|| (((toSet & Setattrs.SETATTR_MODE.getNumber()) > 0) && ((stat
						.getMode() & (1 << 10))) > 0)) {
			metadataCache.invalidate(path);
		} else {
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#nnlink(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces.RPC .UserCredentials, java.lang.String)
	 */
	@Override
	public void unlink(UserCredentials userCredentials, String path)
			throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		// 1. Delete file at MRC.
		unlinkRequest request = unlinkRequest.newBuilder().setPath(path)
				.setVolumeName(volumeName).build();

		unlinkResponse response = RPCCaller
				.<unlinkRequest, unlinkResponse> syncCall(SERVICES.MRC,
						userCredentials, authBogus, volumeOptions,
						uuidResolver, mrcUUIDIterator, false, request,
						new CallGenerator<unlinkRequest, unlinkResponse>() {
							@Override
							public RPCResponse<unlinkResponse> executeCall(
									InetSocketAddress server, Auth authHeader,
									UserCredentials userCreds,
									unlinkRequest input) throws IOException {
								return mrcServiceClient.unlink(server,
										authHeader, userCreds, input);
							}
						});
	...
	private void unlinkAtOsd(FileCredentials fc, String path)
			throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		unlink_osd_Request request = unlink_osd_Request.newBuilder()
				.setFileCredentials(fc).setFileId(fc.getXcap().getFileId())
				.build();
		...
		for (int i = 0; i < fc.getXlocs().getReplicasCount(); i++) {
			osdUuidIterator.clearAndAddUUID(Helper.getOSDUUIDFromXlocSet(
					fc.getXlocs(), i, 0));

			RPCCaller.<unlink_osd_Request, emptyResponse> syncCall(
					...
					new CallGenerator<unlink_osd_Request, emptyResponse>() {
						...
						public RPCResponse<emptyResponse> executeCall(
								InetSocketAddress server, Auth authHeader,
								UserCredentials userCreds,
								unlink_osd_Request input) throws IOException {
							return osdServiceClient.unlink(server, authHeader,
									userCreds, input);
						}
					});
		}
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#rename(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces.RPC .UserCredentials, java.lang.String,
	 * java.lang.String)
	 */
	@Override
	public void rename(UserCredentials userCredentials, String path,
			String newPath) throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		...
		renameRequest request = renameRequest.newBuilder()
				.setVolumeName(volumeName).setSourcePath(path)
				.setTargetPath(newPath).build();

		renameResponse response = RPCCaller
				.<renameRequest, renameResponse> syncCall(SERVICES.MRC,
						userCredentials, authBogus, volumeOptions,
						uuidResolver, mrcUUIDIterator, false, request,
						new CallGenerator<renameRequest, renameResponse>() {
							@Override
							public RPCResponse<renameResponse> executeCall(
									InetSocketAddress server, Auth authHeader,
									UserCredentials userCreds,
									renameRequest input) throws IOException {
								return mrcServiceClient.rename(server,
										authHeader, userCreds, input);
							}
						});
		...
		metadataCache.updateStatTime(newPath, response.getTimestampS(),
				Setattrs.SETATTR_CTIME.getNumber());
	 ...
	 */
	@Override
	public void createDirectory(UserCredentials userCredentials, String path,
			int mode) throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		mkdirRequest request = mkdirRequest.newBuilder()
				.setVolumeName(volumeName).setPath(path).setMode(mode).build();

		timestampResponse response = RPCCaller
				.<mkdirRequest, timestampResponse> syncCall(SERVICES.MRC,
						userCredentials, authBogus, volumeOptions,
						uuidResolver, mrcUUIDIterator, false, request,
						new CallGenerator<mkdirRequest, timestampResponse>() {
							...
							public RPCResponse<timestampResponse> executeCall(
									InetSocketAddress server, Auth authHeader,
									UserCredentials userCreds,
									mkdirRequest input) throws IOException {
								return mrcServiceClient.mkdir(server,
										authHeader, userCreds, input);
							}
						});
	 ...
	 */
	@Override
	public void removeDirectory(UserCredentials userCredentials, String path)
			throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		rmdirRequest request = rmdirRequest.newBuilder()
				.setVolumeName(volumeName).setPath(path).build();

		timestampResponse response = RPCCaller
				.<rmdirRequest, timestampResponse> syncCall(SERVICES.MRC,
						userCredentials, authBogus, volumeOptions,
						uuidResolver, mrcUUIDIterator, false, request,
						new CallGenerator<rmdirRequest, timestampResponse>() {
							...
							public RPCResponse<timestampResponse> executeCall(
									InetSocketAddress server, Auth authHeader,
									UserCredentials userCreds,
									rmdirRequest input) throws IOException {
								return mrcServiceClient.rmdir(server,
										authHeader, userCreds, input);
							}
						});
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#readDir(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces.RPC .UserCredentials, java.lang.String, long,
	 * long, boolean)
	 */
	@Override
	public DirectoryEntries readDir(UserCredentials userCredentials,
			String path, int offset, int count, boolean namesOnly)
			throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		...
		DirectoryEntries.Builder dirEntriesBuilder = DirectoryEntries
				.newBuilder();
		...
		for (int currentOffset = offset; currentOffset < offset + count; offset += volumeOptions
			...
			int limitDirEntriesCount = (currentOffset > offset + count) ? (currentOffset
					- offset - count)
					: volumeOptions.getReaddirChunkSize();

			readdirRequest request = readdirRequest.newBuilder().setPath(path)
					.setVolumeName(volumeName).setNamesOnly(namesOnly)
					.setKnownEtag(0)
					.setSeenDirectoryEntriesCount(currentOffset)
					.setLimitDirectoryEntriesCount(limitDirEntriesCount)
					.build();

			result = RPCCaller.<readdirRequest, DirectoryEntries> syncCall(
					SERVICES.MRC, userCredentials, authBogus, volumeOptions,
					uuidResolver, mrcUUIDIterator, false, request,
					new CallGenerator<readdirRequest, DirectoryEntries>() {
						@Override
						public RPCResponse<DirectoryEntries> executeCall(
								InetSocketAddress server, Auth authHeader,
								UserCredentials userCreds, readdirRequest input)
								throws IOException {
							return mrcServiceClient.readdir(server, authHeader,
									userCreds, input);
						}
					});
			...
			if (result.getEntriesCount() < (currentOffset + volumeOptions
					.getReaddirChunkSize())) {
				break;
			}
		}
		...
		for (int i = 0; i < minimum; i++) {
			if (result.getEntries(i).getStbuf().getNlink() > 1) { // Do not
																	// cache
																	// hard
																	// links.
				metadataCache.invalidate(path);
			} else {
				metadataCache.updateStat(Helper.concatenatePath(path, result
						.getEntries(i).getName()), result.getEntries(i)
						.getStbuf());
			}
		}
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#listXAttrs(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces .RPC.UserCredentials, java.lang.String)
	 */
	@Override
	public listxattrResponse listXAttrs(UserCredentials userCredentials,
			String path) throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
	...
	@Override
	public listxattrResponse listXAttrs(UserCredentials userCredentials,
			String path, boolean useCache) throws IOException,
			PosixErrorException, AddressToUUIDNotFoundException {
		...
		listxattrRequest request = listxattrRequest.newBuilder()
				.setVolumeName(volumeName).setPath(path).setNamesOnly(false)
				.build();

		listxattrResponse response = RPCCaller
				.<listxattrRequest, listxattrResponse> syncCall(
						SERVICES.MRC,
						userCredentials,
						authBogus,
						volumeOptions,
						...
						new CallGenerator<listxattrRequest, listxattrResponse>() {
							@Override
							public RPCResponse<listxattrResponse> executeCall(
									InetSocketAddress server, Auth authHeader,
									UserCredentials userCreds,
									listxattrRequest input) throws IOException {
								return mrcServiceClient.listxattr(server,
										authHeader, userCreds, input);
							}
						});
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#setXAttr(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces.RPC .UserCredentials, java.lang.String,
	 * java.lang.String, java.lang.String,
	 * org.xtreemfs.pbrpc.generatedinterfaces.MRC.XATTR_FLAGS)
	 */
	@Override
	public void setXAttr(UserCredentials userCredentials, String path,
			String name, String value, int flags) throws IOException,
			PosixErrorException, AddressToUUIDNotFoundException {
		setxattrRequest request = setxattrRequest.newBuilder()
				.setVolumeName(volumeName).setPath(path).setName(name)
				.setValue(value).setFlags(flags).build();

		timestampResponse response = RPCCaller
				.<setxattrRequest, timestampResponse> syncCall(
						SERVICES.MRC,
						userCredentials,
						authBogus,
						volumeOptions,
						...
						new CallGenerator<setxattrRequest, timestampResponse>() {
							...
							public RPCResponse<timestampResponse> executeCall(
									InetSocketAddress server, Auth authHeader,
									UserCredentials userCreds,
									setxattrRequest input) throws IOException {
								return mrcServiceClient.setxattr(server,
										authHeader, userCreds, input);
							}
						});
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#getXAttr(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces.RPC .UserCredentials, java.lang.String,
	 * java.lang.String, java.lang.String)
	 */
	@Override
	public String getXAttr(UserCredentials userCredentials, String path,
			String name) throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		...
		if (xtreemfsAttrRequest) {
			...
			// list.
			getxattrRequest request = getxattrRequest.newBuilder()
					.setVolumeName(volumeName).setPath(path).setName(name)
					.build();

			getxattrResponse response = RPCCaller
					.<getxattrRequest, getxattrResponse> syncCall(
							SERVICES.MRC,
							userCredentials,
							authBogus,
							volumeOptions,
							...
							new CallGenerator<getxattrRequest, getxattrResponse>() {
								@Override
								public RPCResponse<getxattrResponse> executeCall(
										InetSocketAddress server,
										Auth authHeader,
										UserCredentials userCreds,
										getxattrRequest input)
										throws IOException {
									return mrcServiceClient.getxattr(server,
											authHeader, userCreds, input);
								}
							});
			...
			Tupel<String, Boolean> cachedXattr = metadataCache.getXAttr(path,
					name);
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#getXAttrSize(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces .RPC.UserCredentials, java.lang.String,
	 * java.lang.String, int)
	 */
	@Override
	public int getXAttrSize(UserCredentials userCredentials, String path,
			String name) throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		...
		boolean xtreemfsAttributeRequested = name.subSequence(0, 9).equals(
				"xtreemfs.");
		...
		if (xtreemfsAttributeRequested) {
			...
			// user attribute
			Tupel<Integer, Boolean> cachedXattrSize = metadataCache
					.getXAttrSize(path, name);
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#removeXAttr(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces .RPC.UserCredentials, java.lang.String,
	 * java.lang.String)
	 */
	@Override
	public void removeXAttr(UserCredentials userCredentials, String path,
			String name) throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		removexattrRequest request = removexattrRequest.newBuilder()
				.setVolumeName(volumeName).setPath(path).setName(name).build();

		RPCCaller.<removexattrRequest, timestampResponse> syncCall(
				SERVICES.MRC, userCredentials, authBogus, volumeOptions,
				uuidResolver, mrcUUIDIterator, false, request,
				new CallGenerator<removexattrRequest, timestampResponse>() {
					...
					public RPCResponse<timestampResponse> executeCall(
							InetSocketAddress server, Auth authHeader,
							UserCredentials userCreds, removexattrRequest input)
							throws IOException {
						return mrcServiceClient.removexattr(server, authHeader,
								userCreds, input);
					}
				});
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#addReplica(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces .RPC.UserCredentials, java.lang.String,
	 * org.xtreemfs.pbrpc.generatedinterfaces.GlobalTypes.Replica)
	 */
	@Override
	public void addReplica(UserCredentials userCredentials, String path,
			Replica newReplica) throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		xtreemfs_replica_addRequest request = xtreemfs_replica_addRequest
				.newBuilder().setVolumeName(volumeName).setPath(path)
				.setNewReplica(newReplica).build();

		RPCCaller
				.<xtreemfs_replica_addRequest, emptyResponse> syncCall(
						SERVICES.MRC,
						userCredentials,
						authBogus,
						volumeOptions,
						...
						new CallGenerator<xtreemfs_replica_addRequest, emptyResponse>() {
							...
							public RPCResponse<emptyResponse> executeCall(
									InetSocketAddress server, Auth authHeader,
									UserCredentials userCreds,
									xtreemfs_replica_addRequest input)
									throws IOException {
								return mrcServiceClient.xtreemfs_replica_add(
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#listReplicas(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces .RPC.UserCredentials, java.lang.String)
	 */
	@Override
	public Replicas listReplicas(UserCredentials userCredentials, String path)
			throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		xtreemfs_replica_listRequest request = xtreemfs_replica_listRequest
				.newBuilder().setVolumeName(volumeName).setPath(path).build();

		Replicas response = RPCCaller
				.<xtreemfs_replica_listRequest, Replicas> syncCall(
						SERVICES.MRC,
						userCredentials,
						authBogus,
						volumeOptions,
						...
						new CallGenerator<xtreemfs_replica_listRequest, Replicas>() {
							@Override
							public RPCResponse<Replicas> executeCall(
									InetSocketAddress server, Auth authHeader,
									UserCredentials userCreds,
									xtreemfs_replica_listRequest input)
									throws IOException {
								return mrcServiceClient.xtreemfs_replica_list(
										server, authHeader, userCreds, input);
							}
						});
	 ...
	 * 
	 * @see
	 * org.xtreemfs.common.libxtreemfs.Volume#removeReplica(org.xtreemfs.foundation
	 * .pbrpc.generatedinterfaces .RPC.UserCredentials, java.lang.String,
	 * java.lang.String)
	 */
	@Override
	public void removeReplica(UserCredentials userCredentials, String path,
			String osdUuid) throws IOException, PosixErrorException, AddressToUUIDNotFoundException {
		// remove the replica
		xtreemfs_replica_removeRequest request = xtreemfs_replica_removeRequest
				.newBuilder().setVolumeName(volumeName).setPath(path)
				.setOsdUuid(osdUuid).build();

		FileCredentials response = RPCCaller
				.<xtreemfs_replica_removeRequest, FileCredentials> syncCall(
						SERVICES.MRC,
						userCredentials,
						authBogus,
						volumeOptions,
						...
						false,
						request,
						new CallGenerator<xtreemfs_replica_removeRequest, FileCredentials>() {
							@Override
							public RPCResponse<FileCredentials> executeCall(
									InetSocketAddress server, Auth authHeader,
									UserCredentials userCreds,
									xtreemfs_replica_removeRequest input)
									throws IOException {
								return mrcServiceClient
										.xtreemfs_replica_remove(server,
												authHeader, userCreds, input);
							}
						});
		...
		unlink_osd_Request request2 = unlink_osd_Request.newBuilder()
				.setFileId(response.getXcap().getFileId())
				.setFileCredentials(response).build();

		RPCCaller.<unlink_osd_Request, emptyResponse> syncCall(SERVICES.OSD,
				userCredentials, authBogus, volumeOptions, uuidResolver,
				mrcUUIDIterator, false, request2,
				new CallGenerator<unlink_osd_Request, emptyResponse>() {
					@SuppressWarnings("unchecked")
					@Override
					public RPCResponse<emptyResponse> executeCall(
							InetSocketAddress server, Auth authHeader,
							UserCredentials userCreds, unlink_osd_Request input)
							throws IOException {
						return osdServiceClient.unlink(server, authHeader,
								userCreds, input);
					}
	 ...
	 */
	@Override
	public List<String> getSuitableOSDs(UserCredentials userCredentials,
			String path, int numberOfOsds) throws IOException,
			PosixErrorException, AddressToUUIDNotFoundException {
		xtreemfs_get_suitable_osdsRequest request = xtreemfs_get_suitable_osdsRequest
				.newBuilder().setVolumeName(volumeName).setPath(path)
				...
				.<xtreemfs_get_suitable_osdsRequest, xtreemfs_get_suitable_osdsResponse> syncCall(
						...
						new CallGenerator<xtreemfs_get_suitable_osdsRequest, xtreemfs_get_suitable_osdsResponse>() {
							...
							public RPCResponse<xtreemfs_get_suitable_osdsResponse> executeCall(
									...
									throws IOException {
								return mrcServiceClient
										.xtreemfs_get_suitable_osds(server,
												authHeader, userCreds, input);
							}
						});
	 ...
	 */
	protected void closeFile(long fileId, FileInfo fileInfo,
			FileHandleImplementation fileHandle) {
		// TODO: Implement this method correctly.
	}

	/**
	 * Returns FileInfo object. Looking for it at first in the openFileTable and
	 * if it is not found there it creates a new one.
	 * 
	 * @return FileInfo
	 */
	private FileInfo getOrCreateFileInfo(long fileId, String path,
			boolean replicateOnClose, XLocSet xlocset) {
		...
		if (fileInfo != null) {
			...
			fileInfo = new FileInfo(this, fileId, path, replicateOnClose,
					xlocset, clientUuid);
			...
			if (Logging.isDebug()) {
				Logging.logMessage(
						...
						Category.misc,
						this,
						"Created a new FileInfo" + " object for the fileId: %s",
						fileId);
			}
			return fileInfo;
		}
