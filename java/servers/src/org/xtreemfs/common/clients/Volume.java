...
public class Volume {
    ...
    static {
        emptyCoordinates = VivaldiCoordinates.newBuilder().setLocalError(0).setXCoordinate(0).setYCoordinate(
            0).build();
    }
    ...
    public String[] list(String path, UserCredentials userCreds) throws IOException {
        ...
        try {
            response = mrcClient.readdir(null, RPCAuthentication.authNone, userCreds, fixedVol, fixedPath, 0,
                0, true, 0);
        ...
        try {
            ...
            for (int i = 0; i < list.length; i++) {
                ...
                if (r != null && r.hasTruncateEpoch()) {
                    ...
                    if ((r.getTruncateEpoch() > s.getTruncateEpoch())
                        || (r.getTruncateEpoch() == s.getTruncateEpoch())
                        && (r.getSizeInBytes() > s.getSize())) {
                        s = s.toBuilder().setSize(r.getSizeInBytes()).setTruncateEpoch(r.getTruncateEpoch())
                                .build();
    ...
    public long getFreeSpace(UserCredentials userCreds) throws IOException {
        ...
        try {
            response = mrcClient.statvfs(null, RPCAuthentication.authNone, userCreds, volumeName.replace("/",
                ""), 0);
    ...
    public void setDefaultReplicationFactor(int replicationFactor, UserCredentials userCreds)
        throws IOException {
    ...
    Stat stat(String path, UserCredentials userCreds) throws IOException {
        ...
        try {
            ...
            if (r != null && r.hasTruncateEpoch()) {
                // update with local file size, if cahced
                if ((r.getTruncateEpoch() > s.getTruncateEpoch())
                    || (r.getTruncateEpoch() == s.getTruncateEpoch()) && (r.getSizeInBytes() > s.getSize())) {
                    s = s.toBuilder().setSize(r.getSizeInBytes()).setTruncateEpoch(r.getTruncateEpoch())
                            .build();
                }
            }
    ...
    void mkdir(String path, int permissions, UserCredentials userCreds) throws IOException {
        RPCResponse response = null;
        try {
            response = mrcClient.mkdir(null, RPCAuthentication.authNone, userCreds, fixPath(volumeName),
                fixPath(path), permissions);
            response.get();
        } catch (PBRPCException ex) {
            throw wrapException(ex);
        } catch (InterruptedException ex) {
    ...
    void touch(String path, UserCredentials userCreds) throws IOException {
        RPCResponse response = null;
        try {
            response = mrcClient.open(null, RPCAuthentication.authNone, userCreds, fixPath(volumeName),
                fixPath(path), GlobalTypes.SYSTEM_V_FCNTL.SYSTEM_V_FCNTL_H_O_CREAT.getNumber(), 0700, 0,
                emptyCoordinates);
            response.get();
        } catch (PBRPCException ex) {
            throw wrapException(ex);
    ...
    void rename(String src, String dest, UserCredentials userCreds) throws IOException {
        RPCResponse response = null;
        try {
            response = mrcClient.rename(null, RPCAuthentication.authNone, userCreds, fixPath(volumeName),
                fixPath(src), fixPath(dest));
            response.get();
        } catch (PBRPCException ex) {
            throw wrapException(ex);
        } catch (InterruptedException ex) {
    ...
    void unlink(String path, UserCredentials userCreds) throws IOException {
        ...
        try {
            response = mrcClient.unlink(null, RPCAuthentication.authNone, userCreds, fixPath(volumeName),
                fixPath(path));
    ...
    void pushFileSizeUpdate(String fileId, UserCredentials userCreds) throws IOException {
        ...
        if (owr != null) {
            ...
            try {
                ...
                xtreemfs_update_file_sizeRequest fsBuf = xtreemfs_update_file_sizeRequest.newBuilder()
                        .setXcap(cap).setOsdWriteResponse(osdResp).build();
                
                response = mrcClient.xtreemfs_update_file_size(null, RPCAuthentication.authNone, userCreds,
                    fsBuf);
                response.get();
    ...
    void closeFile(RandomAccessFile file, String fileId, boolean readOnly, UserCredentials userCreds)
        ...
        try {
            ...
            try {
                response = mrcClient.xtreemfs_update_file_size(null, RPCAuthentication.authNone, userCreds,
                    cap, OSDWriteResponse.newBuilder().build(), true, emptyCoordinates);
                response.get();
            } catch (Exception ex) {
    ...
    RandomAccessFile openFile(File parent, int flags, int mode, UserCredentials userCreds) throws IOException {
        ...
        try {
            response = mrcClient.open(null, RPCAuthentication.authNone, userCreds, fixedVol, fixedPath,
                flags, mode, 0, emptyCoordinates);
            ...
            boolean rdOnly = cred.getXlocs().getReplicaUpdatePolicy().equals(
    ...
    XCap truncateFile(String fileId, UserCredentials userCreds) throws IOException {
        ...
        try {
            response = mrcClient.ftruncate(null, RPCAuthentication.authNone, userCreds, ofl
                    .getCapability(fileId));
            return response.get();
    ...
    List<String> getSuitableOSDs(File file, int numOSDs, UserCredentials userCreds) throws IOException {
        ...
        try {
            ...
            response = mrcClient.xtreemfs_get_suitable_osds(null, RPCAuthentication.authNone, userCreds,
                request);
    ...
    void addReplica(File file, int width, List<String> osdSet, int flags, UserCredentials userCreds)
        throws IOException {
        ...
        try {
            ...
            org.xtreemfs.pbrpc.generatedinterfaces.GlobalTypes.Replica newReplica = org.xtreemfs.pbrpc.generatedinterfaces.GlobalTypes.Replica
                    .newBuilder().addAllOsdUuids(osdSet).setReplicationFlags(flags).setStripingPolicy(sp)
                    .build();
            ...
            xtreemfs_replica_addRequest request = xtreemfs_replica_addRequest.newBuilder().setNewReplica(
                newReplica).setFileId(oldCreds.getXcap().getFileId()).build();
            ...
            if (readOnlyRepl) {
                if ((flags & GlobalTypes.REPL_FLAG.REPL_FLAG_FULL_REPLICA.getNumber()) > 0) {
                    ...
                    response1 = mrcClient.open(null, RPCAuthentication.authNone, userCreds, fixedVol,
                        fixedPath, 0, GlobalTypes.SYSTEM_V_FCNTL.SYSTEM_V_FCNTL_H_O_RDWR.getNumber(), 0,
                        emptyCoordinates);
                    ...
                    for (int objNo = 0; objNo < width; objNo++) {
                        ...
                        response3 = osdClient.read(osd.getAddress(), RPCAuthentication.authNone,
                            RPCAuthentication.userService, newCreds, newCreds.getXcap().getFileId(), objNo,
                            0, 0, 1);
                        response3.get();
                        response3.freeBuffers();
                        response3 = null;
                ...
                response1 = mrcClient.open(null, RPCAuthentication.authNone, userCreds, fixedVol, fixedPath,
                    0, GlobalTypes.SYSTEM_V_FCNTL.SYSTEM_V_FCNTL_H_O_RDWR.getNumber(), 0, emptyCoordinates);
                FileCredentials newCreds = response1.get().getCreds();
    ...
    void removeReplica(File file, String headOSDuuid, UserCredentials userCreds) throws IOException {
        ...
        try {
            ...
            xtreemfs_replica_removeRequest request = xtreemfs_replica_removeRequest.newBuilder().setOsdUuid(
                headOSDuuid).setFileId(oldCreds.getXcap().getFileId()).build();
            
            response2 = mrcClient.xtreemfs_replica_remove(null, RPCAuthentication.authNone, userCreds,
                request);
            ...
            FileCredentials newCreds = FileCredentials.newBuilder().setXcap(delCap.getXcap()).setXlocs(
                oldCreds.getXlocs()).build();
            response3 = osdClient.unlink(osd.getAddress(), RPCAuthentication.authNone,
                RPCAuthentication.userService, newCreds, oldCreds.getXcap().getFileId());
            response3.get();
