...
public class BufferBackedStripingPolicy extends BufferBackedMetadata implements StripingPolicy {
    ...
    private static final int PARITY_WIDTH_INDEX   = 8;
    ...
    public BufferBackedStripingPolicy(byte[] buffer, int offset, int len) {
        ...
        tmp = ByteBuffer.wrap(buffer, offset + PARITY_WIDTH_INDEX, Integer.SIZE / 8);
        this.parity_width = tmp.getInt();
    }
    ...
    public BufferBackedStripingPolicy(String pattern, int stripeSize, int width, int parity_width) {
        ...
        tmp.putInt(stripeSize).putInt(width).putInt(parity_width).put(pattern.getBytes());

        this.pattern = pattern;
        this.stripeSize = stripeSize;
        this.width = width;
        this.parity_width = parity_width;
    }
