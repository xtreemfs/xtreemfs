//automatically generated from MRC.proto at Mon Feb 12 11:33:32 CET 2018
//(c) 2018. See LICENSE file for details.
...
public class MRCServiceClient {
    ...
    public RPCResponse xtreemfs_replica_mark_complete(InetSocketAddress server, Auth authHeader, UserCredentials userCreds, MRC.xtreemfs_replica_mark_completeRequest input) throws IOException {
         if (server == null) server = defaultServer;
         if (server == null) throw new IllegalArgumentException("defaultServer must be set in constructor if you want to pass null as server in calls");
         RPCResponse response = new RPCResponse(null);
    ...
    public RPCResponse xtreemfs_replica_mark_complete(InetSocketAddress server, Auth authHeader, UserCredentials userCreds, String file_id, String osd_uuid) throws IOException {
         final MRC.xtreemfs_replica_mark_completeRequest msg = MRC.xtreemfs_replica_mark_completeRequest.newBuilder().setFileId(file_id).setOsdUuid(osd_uuid).build();
         return xtreemfs_replica_mark_complete(server, authHeader, userCreds,msg);
    }
