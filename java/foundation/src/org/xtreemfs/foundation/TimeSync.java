...
/**
 * A class that offers a local time w/ adjustable granularity and a global time based on the time reported by
 * the DIR. Global time is adjusted periodically. This class should be used to minimize the number of calls to
 * System.currentTimeMillis which is a costly system call on Linux. Moreover it offers a system-global time.
 * 
 * @author bjko
 */
public final class TimeSync extends LifeCycleThread {
    ...
    /**
     * The maximum round trip time for a clock synchronization message between the <code>TimeSync</code> and
     * the DIR. If the round trip time of a synchronization message exceeds this value, the message will be
     * ignored.
     */
     ...
     */
    private TimeSync(ExtSyncSource source, TimeServerClient dir, InetSocketAddress gpsd, int timeSyncInterval,
            int localTimeRenew) {
        super("TSync Thr");
        setDaemon(true);
     ...
     */
    public synchronized void init(ExtSyncSource source, TimeServerClient dir, InetSocketAddress gpsd,
            int timeSyncInterval, int localTimeRenew) {
    ...
    /**
     * Initializes the time synchronizer. Note that only the first invocation of this method has an effect,
     * any further invocations will be ignored.
     * 
     * @param dir
     * @param timeSyncInterval
     * @param localTimeRenew
    ...
    public static TimeSync initialize(TimeServerClient dir, int timeSyncInterval, int localTimeRenew) throws Exception {
        ...
        if (theInstance != null) {
            Logging.logMessage(Logging.LEVEL_WARN, Category.lifecycle, null, "time sync already running", new Object[0]);
            return theInstance;
        }
    ...
    /**
     * returns the current value of the local system time variable. Has a resolution of localTimeRenew ms.
     */
    ...
    /**
     * returns the current value of the local system time adjusted to global time. Has a resolution of
     * localTimeRenew ms.
     */
     ...
     * milliseconds.
     */
    public static long getPreciseGlobalTime() {
        return System.currentTimeMillis() + getInstance().currentDrift;
    }
    ...
    /**
     * 
     * @return the timestamp (local time) when the drift was calculated
     */
    ...
    private void resync() {
        ...
        case XTREEMFS_DIR: {
            try {
                ...
                if (Math.abs(oldDrift - currentDrift) > 5000 && oldDrift != 0) {
                    Logging.logMessage(Logging.LEVEL_ERROR, Category.misc, this, "STRANGE DRIFT CHANGE from %d to %d",
                            oldDrift, currentDrift);
                }
        ...
        case GPSD: {
            try {
                ...
                Logging.logMessage(Logging.LEVEL_DEBUG, this, "global GPSd time: %d (%d:%d:%d)", c.getTimeInMillis(),
                ...
                Logging.logMessage(Logging.LEVEL_DEBUG, Category.misc, this, "resync success, drift: %d ms",
                        Math.abs(oldDrift - currentDrift));
    ...
    public static void main(String[] args) {
        try {
            ...
            /*
             * RPCNIOSocketClient c = new RPCNIOSocketClient(null, 5000, 60000); c.start();
             * c.waitForStartup(); DIRClient dir = new DIRClient(c, new InetSocketAddress("xtreem.zib.de",
             * 32638));
             */
            ...
            for (;;) {
                Logging.logMessage(Logging.LEVEL_INFO, Category.misc, (Object) null, "local time  = %d",
                        TimeSync.getLocalSystemTime());
                Logging.logMessage(Logging.LEVEL_INFO, Category.misc, (Object) null, "global time = %d + %d",
                        TimeSync.getGlobalTime(), ts.getDrift());
                Thread.sleep(1000);
            }
