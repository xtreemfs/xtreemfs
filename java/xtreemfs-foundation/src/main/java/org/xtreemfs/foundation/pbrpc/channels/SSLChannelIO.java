...
public class SSLChannelIO extends ChannelIO {
                    ...
                    while (inNetBuffer.hasRemaining()) {
                        ...
                        switch (result.getStatus()) {
                            case OK: {
                                ...
                                FIXME: if client doesn't close the connection
        ...
        if (!shutdownInProgress) {
            if (handshakeComplete) {
                ...
                switch (result.getStatus()) {
                    ...
                    default: {
                        throw new IOException("The SSLEngine is in an unknown state.");
                    }
                }
    ...
    public boolean doHandshake(SelectionKey key) throws IOException, CancelledKeyException {
        ...
        if (!handshakeComplete) {
            ...
            switch (handshakeStatus) {
                ...
                default: {
                    throw new IOException("The SSLEngine is in an unknown handshake-state.");
                }
            }
        }
        return handshakeComplete;
    ...
    protected void doTasks(final SelectionKey key) throws CancelledKeyException {
        ...
        switch (handshakeStatus = sslEngine.getHandshakeStatus()) {
            ...
            default: {
                Logging.logMessage(Logging.LEVEL_ERROR, Category.auth, this,
                        "Exception in worker-thread: The SSLEngine is in an unknown handshake-state.");
                assert (false);
            }
        }
    }
