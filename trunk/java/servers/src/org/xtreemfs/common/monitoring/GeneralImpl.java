...
/**
 * This class represents the monitoring information exposed by the SNMP agent regarding to all services.
 * 
 * <br>
 * May 5, 2011
 * 
...
public class GeneralImpl extends General {
    ...
    public String getIsRunning() throws SnmpStatusException {
        if (statusMonitor.getInitiatingService().equals(ServiceTypes.DIR)) {
            //TODO: Since the DIR don't have a heartbeatthread find a method to determine if 
            //the DIR is still alive.
            return "ONLINE";
        }

        if (statusMonitor.getInitiatingService().equals(ServiceTypes.MRC)) {
            //calculate the difference between the last hearbeat an the current time. If these values
