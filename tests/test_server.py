...
class Server:
    ...
    def start(self,
        ...
        if sys.platform == "win32" or not self.is_running():
            ...
            java_args.append("org.xtreemfs." + self.__class__.__name__.lower() + "." + self.__class__.__name__)
...
class Scheduler(Server):
   def configure(self,
       ...
       self._config['osd_autodiscover'] = 'false'       
       self._config['osd_capabilities_file'] = os.path.join(self._data_dir, 'scheduler.osd_capabilities')

       self._config['basefilesize_in_bytes'] = 209715200
   ...
   def create_osd_capabilities_file(self, maxOsds):
      text = '# autogenerated by test_server.py at '+str(datetime.now())+'\n'
      for i in range(maxOsds):
         text += 'test-osd' + str(i) + ';100.0;100.0;100.0,99.0,98.0,97.0,96.0,95.0' + '\n'
      f = open(os.path.join(self._data_dir, 'scheduler.osd_capabilities'), 'a+')
      f.write(text)
      f.close()
