...
do
  set +e
  valgrind --leak-check=full --show-reachable=yes --error-exitcode=23 --gen-suppressions=yes --suppressions="${XTREEMFS_DIR}/cpp/valgrind.supp" --num-callers=100 ./$test | tee -a $VALGRIND_LOG_FILE
  rc=$?
  set -e
