...
do
  set +e
  valgrind --leak-check=full --show-reachable=yes --error-exitcode=23 --suppressions="${XTREEMFS_DIR}/cpp/valgrind.supp" --num-callers=50 --track-origins=yes --gen-suppressions=all ./$test | tee -a $VALGRIND_LOG_FILE
  rc=$?
  set -e
