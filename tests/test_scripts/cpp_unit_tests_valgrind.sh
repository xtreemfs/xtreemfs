...
do
  ...
  set +e
  valgrind --leak-check=full --show-reachable=yes --track-origins=yes --error-exitcode=23 --suppressions="${XTREEMFS_DIR}/cpp/valgrind.supp" --gen-suppressions=all ./$test &>>$VALGRIND_LOG_FILE
  rc=$?
  set -e
