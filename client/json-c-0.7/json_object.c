...
static int json_object_int_to_json_string(struct json_object* this,
					  struct printbuf *pb)
{
  return sprintbuf(pb, "%lld", this->o.c_int);
}

struct json_object* json_object_new_int(int64_t i)
{
...
int64_t json_object_get_int(struct json_object *this)
{
  int64_t cint;
  ...
  case json_type_int:
    return this->o.c_int;
  case json_type_double:
    return (int64_t)this->o.c_double;
  case json_type_boolean:
    return this->o.c_boolean;
  case json_type_string:
    if(sscanf(this->o.c_string, "%lld", &cint) == 1) return cint;
  default:
    return 0;
  }
}
