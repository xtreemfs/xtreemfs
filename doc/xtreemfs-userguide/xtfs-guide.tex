...
This document is \copyright{} 2009-2011 by Bj\"orn Kolbeck, Jan Stender, Michael Berlin, Matthias Noack, Paul Seiferth, Felix Langner, NEC HPC Europe, Felix Hupfeld, Juan Gonzales. All rights reserved.

\setcounter{tocdepth}{2}
\tableofcontents

\chapter*{Changes}
Summary of important changes in release 1.3.2:
...
\begin{itemize}
 ...
 \item \textbf{Support for custom attributes in the OSD Selection policies}\\
    Added support for OSD Selection based on Custom Attributes e.g., assign a country code to every OSD as custom attribute and limit the placement of files on OSDs based on the attribute (see \ref{sec:customOSDSelection}).
\end{itemize}
...
\begin{figure}[h]
 \centering
 \resizebox{\linewidth}{!}{\includegraphics{images/screenshot_status.png}}
...
The replication of the database requires that all records are synchronously written to disk. Therefore, the parameter \texttt{babudb.sync} has to be changed to the value \texttt{SYNC\_WRITE} in the same configuration file \texttt{/etc/xos/xtreemfs/mrcconfig.properties}:
\begin{verbatim}
...
DIR replication is enabled and configured in the exact same way as MRC replication. Change \texttt{/etc/xos/xtreemfs/server-repl-plugin/dir.properties} accordingly to configure the plug-in and set the sync mode to \texttt{SYNC\_WRITE}.

In order to ensure that MRCs and OSDs know all DIR replicas, you have to add the addresses of all DIR replicas to each MRC and OSD configuration file. Specify the first DIR replica in the options \texttt{dir\_service.host} and \texttt{dir\_service.port} and all subsequent ones in the options \texttt{dir\_service.<i>.host} and \texttt{dir\_service.<i>.port} where i starts from zero. For example:

\begin{verbatim}
dir_service.host = dir1.example.com
dir_service.port = 32638
dir_service.0.host = dir2.example.com
dir_service.0.port = 32638
dir_service.1.host = dir3.example.com
dir_service.1.port = 32638
\end{verbatim}

This allows the MRCs and OSDs to fail over to each DIR replica. On the contrary, the XtreemFS client does not support yet to specify multiple DIR replicas and therefore cannot automatically fail over between them.
...
Vivaldi network coordinates are calculated between all OSDs. 
...
There is a built-in visualization for the vivaldi coordinates.
...
\item Configure Hadoop to use XtreemFS instead of HDFS:
	...
	The easiest way to do so is to download XtreemFSHadoopClient.jar into Hadoop's \textit{lib} directory, or to symlink it there.
	...
	\item Now you have to specify some properties in \textit{core-site.xml} (in the \textit{conf} 
...
<configuration>
  ...
  <property>
	...
	\end{enumerate}
	Hint: If you want to provide user rights to your Hadoop installation according to the POSIX file-access-policy, you 
...
\item To provide the minimum JobTracker configuration for Hadoop you also have to add the following property to the 
...
\item Finally you are now able to start the JobTracker and the TaskTracker by running '\textit{bin/start-mapred.sh}' from within the Hadoop root-directory.
...
In particular, you need boost and libssl-dev. On a Unix plattform, you also need libfuse-dev or you have to comment out the respective portions in the CMake configuration manually. At last, you have to install ``cmake'' which generates the build system.
  ...
  \item Check out the XtreemFS trunk.

\begin{verbatim}
svn checkout http://xtreemfs.googlecode.com/svn/trunk/ \
             xtreemfs-read-only 
\end{verbatim}
