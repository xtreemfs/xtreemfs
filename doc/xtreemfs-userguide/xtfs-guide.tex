...
\begin{itemize}
  ...
  \item \textbf{New OSD selection policy}\\
      It is no possible to specify a preferred OSD via a UUID (ID 1003).
...
\item Hadoop (2.2.x or 2.3+ are compatible. Versions 0.23.x, 2.0.x a,d 2.1.x use another version of Google's Protobuf (2.4.0a), however XtreemFS uses 2.5.0 which is incompatible, so rebuilding XtreemFS with version 2.4.0a is required. Versions 0.22 and earlier, as well as 1.x do not use Google's Protobuf, and are compatible as well.) (\href{http://hadoop.apache.org}{hadoop.apache.org})
