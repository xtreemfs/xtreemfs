...
\begin{itemize}
\item XtreemFS servers (v 1.3.1) inlcuding XtreemFS.jar and Foundation.jar (\href{http://www.XtreemFS.org}{www.XtreemFS.org})
...
\item Configure Hadoop to use XtreemFS instead of HDFS:
	\begin{enumerate}
	\item After downloading and extracting Hadoop you first have to add XtreemFS, the HadoopClient and the Foundation package to its 
	classpath. To do so edit the \textit{hadoop-env.sh} that can be found in the \textit{conf} directory of Hadoop and 
	add the paths to XtreemFS.jar, Foundation.jar and HadoopClient.jar separated by '\textit{:}' to the HADOOP\_CLASSPATH. 
	...
	/usr/share/XtreemFS.jar:/usr/share/HadoopClient.jar"
	\end{verbatim}
