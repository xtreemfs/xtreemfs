...
\item The script must print one of the following numbers to stdout:
\begin{itemize}
...
\item \texttt{1} - FAILED: Storage device is in an critical state. The OSD will filtered out when the default OSD selection policy is used.
\item \texttt{2} - WARNING: Storage could be in an critical state soon. The OSD will filtered out when the default OSD selection policy is used.
