...
\end{verbatim}
\texttt{communites} is the SNMP community string your managers have to use and \texttt{managers} is a list of hostname, ip address or subnets in prefix notation (i.e. "xtreemfs-host", "192.168.0.54" or "192.168.0.3/24").
...
When mounting a volume, the client will immediately go into background and won't display any error messages. Use the \texttt{-f} option to prevent the mount process from going into background and get all error messages printed to the console. Alternatively, you can use the \texttt{xtfsutil} to print the last 20 errors for a mounted volume.
...
\begin{itemize}
 ...
 \item \textbf{DNS based OSD Selection (policy ID 3000)}\\
