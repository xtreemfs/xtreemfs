...
public class ReplicationTest extends TestCase {
    ...
    public void testStriped() throws Exception {
        ...
        RPCResponse response = client.put(locations.getOSDsByObject(objectNo).get(2).getAddress(),
                this.locations, this.capability, this.file, objectNo, this.data);
        response.waitForResponse();
        response.freeBuffers();

        // read object from replica 3 (object exists on this OSD) => normal read
        response = client.get(locations.getOSDsByObject(objectNo).get(2).getAddress(), this.locations,
                this.capability, this.file, objectNo);
        response.waitForResponse();
        assertEquals(HTTPUtils.SC_OKAY, response.getStatusCode());
        assertEquals(HTTPUtils.DATA_TYPE.BINARY.toString(), response.getHeaders().getHeader(
        ...
        // => replication
        response = client.get(locations.getOSDsByObject(objectNo).get(1).getAddress(), this.locations,
                this.capability, this.file, objectNo);
        response.waitForResponse();
        assertEquals(HTTPUtils.SC_OKAY, response.getStatusCode());
        assertEquals(HTTPUtils.DATA_TYPE.BINARY.toString(), response.getHeaders().getHeader(
        ...
        // => replication
        response = client.get(locations.getOSDsByObject(objectNo).get(3).getAddress(), this.locations,
                this.capability, this.file, objectNo);
        response.waitForResponse();
        assertEquals(HTTPUtils.SC_OKAY, response.getStatusCode());
        assertEquals(HTTPUtils.DATA_TYPE.BINARY.toString(), response.getHeaders().getHeader(
    ...
    public void testHoleAndEOF() throws Exception {
        ...
        RPCResponse response = client.put(locations.getOSDsByObject(objectNo).get(0).getAddress(),
                this.locations, this.capability, this.file, objectNo, this.data);
        response.waitForResponse();
        response.freeBuffers();
        ...
        response = client.put(locations.getOSDsByObject(objectNo + 2).get(0).getAddress(), this.locations,
                this.capability, this.file, objectNo + 2, 1024 * this.stripeSize / 4, data2);
        response.waitForResponse();
        response.freeBuffers();

        // write half object to replica 1 => half object, EOF
        response = client.put(locations.getOSDsByObject(objectNo + 3).get(0).getAddress(), this.locations,
                this.capability, this.file, objectNo + 3, data2);
        response.waitForResponse();
        response.freeBuffers();
        ...
        response = client.get(locations.getOSDsByObject(objectNo + 1).get(1).getAddress(), this.locations,
                this.capability, this.file, objectNo + 1);
        response.waitForResponse();
        assertEquals(HTTPUtils.SC_OKAY, response.getStatusCode());
        assertEquals(HTTPUtils.DATA_TYPE.BINARY.toString(), response.getHeaders().getHeader(
                HTTPHeaders.HDR_CONTENT_TYPE));
        ...
        response = client.get(locations.getOSDsByObject(objectNo + 4).get(1).getAddress(), this.locations,
                this.capability, this.file, objectNo + 4);
        response.waitForResponse();
        assertEquals(HTTPUtils.SC_OKAY, response.getStatusCode());
        ...
        response = client.get(locations.getOSDsByObject(objectNo + 2).get(1).getAddress(), this.locations,
                this.capability, this.file, objectNo + 2);
        response.waitForResponse();
        assertEquals(HTTPUtils.SC_OKAY, response.getStatusCode());
        assertEquals(HTTPUtils.DATA_TYPE.BINARY.toString(), response.getHeaders().getHeader(
                HTTPHeaders.HDR_CONTENT_TYPE));
        ...
        response = client.get(locations.getOSDsByObject(objectNo + 3).get(1).getAddress(), this.locations,
                this.capability, this.file, objectNo + 3);
        response.waitForResponse();
        assertEquals(HTTPUtils.SC_OKAY, response.getStatusCode());
        assertEquals(HTTPUtils.DATA_TYPE.BINARY.toString(), response.getHeaders().getHeader(
                HTTPHeaders.HDR_CONTENT_TYPE));
    ...
    public void testCorrectFilesize() throws Exception {
        ...
        RPCResponse response = client.put(osd.getAddress(), locations, capability, file, 0, data);
        response.waitForResponse();
        response.freeBuffers();
        // write object to replica 1 : OSD 2
        osd = locations.getOSDsByObject(1).get(0);
        response = client.put(osd.getAddress(), locations, capability, file, 1, data);
        response.waitForResponse();
        response.freeBuffers();
        ...
        // => replication
        response = client.get(locations.getOSDsByObject(0).get(1).getAddress(), this.locations,
        ...
        // => replication
        response = client.get(locations.getOSDsByObject(1).get(2).getAddress(), this.locations,
    ...
    public void testCorrectFilesizeForHole() throws Exception {
        ...
        response = client.put(osd.getAddress(), locations, capability, file, 2, data);
        response.waitForResponse();
        response.freeBuffers();
        ...
        response = client.get(locations.getOSDsByObject(1).get(1).getAddress(), this.locations,
                this.capability, this.file, 1);
        response.waitForResponse();
        assertEquals(HTTPUtils.SC_OKAY, response.getStatusCode());
        // correct length
        ...
        // => replication
        response = client.get(locations.getOSDsByObject(2).get(2).getAddress(), this.locations,
    ...
    public void testCorrectFilesizeForEOF() throws Exception {
        ...
        // => replication, but EOF
        response = client.get(locations.getOSDsByObject(2).get(1).getAddress(), this.locations,
                this.capability, this.file, 2);
        response.waitForResponse();
        assertEquals(HTTPUtils.SC_OKAY, response.getStatusCode());
        // correct length
        ...
        // => replication
        response = client.get(locations.getOSDsByObject(1).get(0).getAddress(), this.locations,
                this.capability, this.file, 1);
        response.waitForResponse();
        assertEquals(HTTPUtils.SC_OKAY, response.getStatusCode());
        // correct length
    ...
    public void testObjectLocalAvailable() throws Exception {
        ...
        RPCResponse tmp = client.put(serverID.getAddress(), locations, capability, file, objectNo, data);
        tmp.waitForResponse();
        tmp.freeBuffers();

        RPCResponse response = client.readLocalRPC(serverID.getAddress(), locations, capability, file,
                objectNo);
        response.waitForResponse();

        assertEquals(HTTPUtils.SC_OKAY, response.getStatusCode());
        assertEquals("" + data.limit() + "", response.getHeaders().getHeader(HTTPHeaders.HDR_XNEWFILESIZE));
        assertEquals(HTTPUtils.DATA_TYPE.BINARY.toString(), response.getHeaders().getHeader(
                HTTPHeaders.HDR_CONTENT_TYPE));
        assertEquals(data.getBuffer(), response.getBody().getBuffer());
        ...
        response = client.checkObject(serverID.getAddress(), locations, capability, file, objectNo);
    ...
    public void testObjectLocalNOTAvailable() throws Exception {
        ...
        assertEquals(HTTPUtils.DATA_TYPE.JSON.toString(), response.getHeaders().getHeader(
                HTTPHeaders.HDR_CONTENT_TYPE));
        HashMap<String, Long> json = (HashMap<String, Long>) JSONParser.parseJSON(new JSONString(new String(
        ...
        response = client.put(serverID.getAddress(), locations, capability, file, objectNo, ReusableBuffer
                .wrap(content.getBytes()));
        response.waitForResponse();
        response.freeBuffers();

        response = client.readLocalRPC(serverID.getAddress(), locations, capability, file, objectNo + 3);
        response.waitForResponse();

        assertEquals(HTTPUtils.SC_OKAY, response.getStatusCode());
        // get correct filesize
        assertEquals(content.getBytes().length + "", response.getHeaders().getHeader(
                HTTPHeaders.HDR_XNEWFILESIZE));
        // get empty map, because object was not available
        assertEquals(HTTPUtils.DATA_TYPE.JSON.toString(), response.getHeaders().getHeader(
                HTTPHeaders.HDR_CONTENT_TYPE));
        json = (HashMap<String, Long>) JSONParser.parseJSON(new JSONString(new String(response.getBody()
                .array())));
        assertEquals(0, json.size());

        // read object that has not been written (hole)
        response = client.put(serverID.getAddress(), locations, capability, file, 1l, ReusableBuffer
                .wrap(content.getBytes()));
        response.waitForResponse();
        response.freeBuffers();
        response = client.put(serverID.getAddress(), locations, capability, file, 10l, ReusableBuffer
                .wrap(content.getBytes()));
        response.waitForResponse();
        response.freeBuffers();

        response = client.readLocalRPC(serverID.getAddress(), locations, capability, file, 5l);
        response.waitForResponse();

        assertEquals(HTTPUtils.SC_OKAY, response.getStatusCode());
        // get correct filesize
        assertEquals((10l * stripeSize * 1024 + content.getBytes().length) + "", response.getHeaders()
