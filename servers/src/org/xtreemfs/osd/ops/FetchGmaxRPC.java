...
public final class FetchGmaxRPC extends Operation {

    public FetchGmaxRPC(RequestDispatcher master) {
        super(master);
    }
    ...
    public void startRequest(OSDRequest rq) {
        ...
        master.getStage(Stages.STORAGE).enqueueOperation(rq,
            StorageThread.STAGEOP_FETCH_GMAX, new StageCallbackInterface() {

                public void methodExecutionCompleted(OSDRequest request,
                    StageResponseCode result) {
                    requestFinished(request, result);
                }
            });
    }

    private void requestFinished(OSDRequest request, StageResponseCode result) {
