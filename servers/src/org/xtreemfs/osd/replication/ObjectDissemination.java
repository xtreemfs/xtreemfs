...
/**
 * Handles the fetching of replicas. 15.09.2008
 * 
 * @author clorenz
 */
public class ObjectDissemination {
     ...
     * request) key: <code>fileID</code>
     */
    private HashMap<String, TransferStrategy> filesInProgress;

    /**
     * counts how many requests are actually downloading objects of this file,
     * maybe in background (without a client request) key: <code>fileID</code>
     */
     ...
     * so it must wait until the object is fetched. key:
     * <code>fileID:objectID</code>
     */
    ...
    /**
     * Creates a new FetchAndWriteReplica request and sets up all necessary
     * infos.
     * 
     * @param details
     * @return
    ...
    private OSDRequest generateFetchAndWriteReplicaRequest(RequestDetails details) {
        ...
        newRq.getDetails().setReplicationTransferStrategy(details.getReplicationTransferStrategy());
    ...
    public boolean prepareRequest(OSDRequest rq) {
        ...
        if (next != null) { // there is something to fetch
            ...
            // // if a original OSDRequest is waiting for this object, attach it
            // to
            // // the request, which should fetch it
            // if
            // (this.waitingOSDRequests.containsKey(rq.getDetails().getFileId()
            ...
            long lastKnownObject = rq.getDetails().getCurrentReplica().getStripingPolicy()
                    .calculateLastObject(strategy.getKnownFilesize());
    ...
    public void sendFetchObjectRequest(OSDRequest rq) {
        ...
        try {
            RPCResponse response = client.readLocalRPC(next.osd.getAddress(), details.getLocationList(),
                    details.getCapability(), details.getFileId(), next.objectID);
            ...
            response.setResponseListener(new RPCResponseListener() {
                ...
                public void responseAvailable(RPCResponse response) {
                    OSDRequest originalRq = (OSDRequest) response.getAttachment();
                    try {
                        // set data
                        originalRq.setData(response.getBody(), HTTPUtils.DATA_TYPE.toDataType(response
                                .getHeaders().getHeader(HTTPHeaders.HDR_CONTENT_TYPE)));
                        // set filesize
                        originalRq.getDetails().setKnownFilesize(
                                Long.valueOf(response.getHeaders().getHeader(HTTPHeaders.HDR_XNEWFILESIZE)));
                    ...
                    stage.enqueueOperation(originalRq, ReplicationStage.STAGEOP_INTERNAL_OBJECT_FETCHED,
         ...
         * ""); speedyRq.registerListener(new SpeedyResponseListener() { public
         * void receiveRequest(SpeedyRequest response) { OSDRequest originalRq =
         * (OSDRequest) response.getOriginalRequest(); // TODO: set attributes
         * originalRq.setData(response.responseBody,
         * HTTPUtils.DATA_TYPE.toDataType(response.responseHeaders
         * .getHeader(HTTPHeaders.HDR_CONTENT_TYPE))); // set filesize long
         * filesize =
         * Long.parseLong(response.responseHeaders.getHeader(HTTPHeaders
         * .HDR_XNEWFILESIZE));
         * originalRq.getDetails().getReplicationTransferStrategy
         * ().setKnownFilesize(filesize); stage.enqueueOperation(originalRq,
         * ReplicationStage.STAGEOP_INTERNAL_OBJECT_FETCHED, originalRq
         * .getCurrentCallback()); } }); try { master.sendSpeedyRequest(rq,
         * speedyRq, next.osd.getAddress()); } catch (IOException e) { // TODO
         * Auto-generated catch block e.printStackTrace(); }
         */
    }

    /**
     * Checks if the object could be fetched. If not, retry.
     * 
     * @param rq
     */
    public boolean objectFetched(OSDRequest rq) {
        ...
        if (rq.getDataType() == HTTPUtils.DATA_TYPE.BINARY) {
            ...
            if (strategy.getKnownFilesize() > 0) { // at least one time a
                // correct filesize was
                // fetched
                StripingPolicy sp = rq.getDetails().getCurrentReplica().getStripingPolicy();
                long currentObject = rq.getDetails().getObjectNumber();
                long lastKnownObject = sp.calculateLastObject(strategy.getKnownFilesize());
            ...
            Logging.logMessage(Logging.LEVEL_TRACE, this, "object not fetched => try next replica for "
             ...
             * strategy.addPreferredObject(next.objectID); else
    ...
    public void triggerNewRequests(OSDRequest rq) {
        ...
        /*
         * for (int i = 0; i < 4 && this.requestID > -20; i++) { OSDRequest
         * newRq = generateWriteReplicaRequest(rq.getDetails());
         * prepareRequest(newRq); // start new request
         * newRq.getOperation().startRequest(newRq); }
         */
        // update maps
