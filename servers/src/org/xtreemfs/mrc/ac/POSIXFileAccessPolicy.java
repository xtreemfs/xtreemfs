...
public class POSIXFileAccessPolicy implements FileAccessPolicy {
    ...
    private SliceManager        sliceMan;
    
    public POSIXFileAccessPolicy(SliceManager sliceMan) {
    ...
    public void checkPermission(String volumeId, long fileId, long parentId, String userId,
        ...
        try {
            ...
            // rights
            ACLEntry entry = getRelevantACLEntry(volumeId, file, parent, userId, groupIds,
                accessMode);
            assert (entry != null);
            ...
            if (checkRights(accessMode, entry.getRights(), file, parent, userId)
                && (maskEntry == null || checkRights(accessMode, maskEntry.getRights(), file,
                    parent, userId)))
                return;
            else
                accessDenied(volumeId, fileId, accessMode);
    ...
    public void checkSearchPermission(String volumeId, String path, String userId,
        ...
        try {
            ...
            StorageManager sMan = sliceMan.getSliceDB(volumeId, 1, 'r');
            ...
            for (int index = 0; index < path.length();) {
                ...
                sMan = sliceMan.getSliceDB(volumeId, path.substring(0, newIndex - 1), 'r');
                
                parentDir = sMan.getChild(nextComponent, parentDir.getId());
                checkPermission(volumeId, parentDir.getId(), 0, userId, groupIds, "x");
    ...
    public void setACLEntries(String volumeId, long fileId, String userId, List<String> groupIDs,
        ...
        try {
            ...
            Map<String, Object> acl = Converter.aclToMap(sMan.getFileEntity(fileId).getAcl());
    ...
    public Map<String, Object> createDefaultVolumeACL(String volumeId) throws BrainException {
        ...
        Map<String, Object> aclMap = new HashMap<String, Object>();
        aclMap.put(OWNER, 511L); // the owner may do anything
        aclMap.put(OWNER_GROUP, 0L);
        aclMap.put(OTHER, 0L);
    ...
    public void setPosixAccessRights(String volumeId, long fileId, String userId,
        List<String> groupIds, long posixAccessRights) throws BrainException, UserException {
