 ...
 * @author stender
 */
public class GetProtocolVersionOperation extends MRCOperation {
    
    static class Args {
        public List<Long> proposedVersions;
    }
    
    public static final String RPC_NAME = "getProtocolVersion";
    
    public GetProtocolVersionOperation(MRCRequestDispatcher master) {
        super(master);
    }
    ...
    public void startRequest(MRCRequest rq) {
        ...
        try {
            long result = VersionManagement
                    .getMatchingProtVers(((Args) rq.getRequestArgs()).proposedVersions);
            if (result == -1)
                finishRequest(rq, new ErrorRecord(ErrorClass.BAD_REQUEST,
                    "No matching protocol version found. Server supports "
                        + VersionManagement.getSupportedProtVersAsString()));
            else {
                rq.setData(ReusableBuffer.wrap(Long.toString(result).getBytes()));
                rq.setDataType(DATA_TYPE.JSON);
                finishRequest(rq);
            }
    ...
    public ErrorRecord parseRPCBody(MRCRequest rq, List<Object> arguments) {
        ...
        try {
            ...
            args.proposedVersions = (List<Long>) arguments.get(0);
            if (arguments.size() == 1)
                return null;
