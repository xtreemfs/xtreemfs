...
public class ServiceRegistrySet extends ArrayList<ServiceRegistry>
{

    public ServiceRegistrySet()
    { }

    public ServiceRegistrySet( Object from_array )
    {
    ...
    public Object serialize() 
    {
        ...
        {
            ServiceRegistry next_value = get( value_i );                    
    ...
    public void serialize(ONCRPCBufferWriter writer) {
        ...
        for ( Iterator<ServiceRegistry> i = iterator(); i.hasNext(); )
        {
            ServiceRegistry next_value = i.next();        
    ...
    public void deserialize( Object[] from_array )
    {
        for ( int from_array_i = 0; from_array_i < from_array.length; from_array_i++ )
            this.add( new ServiceRegistry( from_array[from_array_i] ) );
    }        
    ...
    if (new_size > org.xtreemfs.interfaces.utils.XDRUtils.MAX_ARRAY_ELEMS)
        ...
        {
            ServiceRegistry new_value; new_value = new ServiceRegistry(); new_value.deserialize( buf );;
            this.add( new_value );
        }
    } 
