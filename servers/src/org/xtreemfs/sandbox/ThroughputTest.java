...
public class ThroughputTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final Capability cap = new Capability(fileId, "w", 0, secret);

        String osdsAsList = "";
        for (int i = 0; i < osds.size(); i++)
            osdsAsList += "\"http://" + osds.get(i) + "\""
                + (i < osds.size() - 1 ? ", " : "");

        Locations loc = new Locations(new JSONString(
            "[[[{\"policy\": \"RAID0\", \"stripe-size\":" + stripeSize
                + ", \"width\":" + osds.size() + " }, [" + osdsAsList
                + "]]], 1, \"sync\"]"));
        ...
        for (long i = 0; i < fileSize / stripeSize; i++) {
            ...
            try {
                ...
                if (write) {
                    ...
                    final RPCResponse res = clients[client].put(uris[osd], loc,
                ...
                // send read requests
                    ...
                    final RPCResponse res = clients[client].get(uris[osd], loc,
                        cap, fileId, i);
                    res.setResponseListener(new RPCResponseListener() {
                        public void responseAvailable(RPCResponse response) {
                            synchronized (lock) {
