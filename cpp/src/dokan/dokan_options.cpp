...
DokanOptions::DokanOptions() : Options(), dokan_descriptions_("Dokan Options") {
  // Dokan options.
  helptext_usage_ =
...
void DokanOptions::ParseCommandLine(int argc, char** argv) {
  ...
  all_descriptions_.add(mount).add(dokan_descriptions_);
  po::variables_map vm;
  try {
...
std::string DokanOptions::ShowCommandLineUsage() {
  return helptext_usage_
      + "\nFor complete list of options, please specify -h or --help.\n";
}

std::string DokanOptions::ShowCommandLineHelp() {
