...
TEST_F(UserMappingGridmapUnicore6Test, TestBasicDNAndOUResolving) {
  EXPECT_EQ("CN=Patrick Schaefer,OU=CSR,OU=Konrad-Zuse-Zentrum fuer Informationstechnik Berlin (ZIB),O=GridGermany,C=DE",
            user_mapping_->UIDToUsername(0));
  EXPECT_EQ(0,
            user_mapping_->UsernameToUID("CN=Patrick Schaefer,OU=CSR,OU=Konrad-Zuse-Zentrum fuer Informationstechnik Berlin (ZIB),O=GridGermany,C=DE"));
...
TEST_F(UserMappingGridmapUnicore6Test, GridmapFileReload) {
  EXPECT_EQ("CN=Patrick Schaefer,OU=CSR,OU=Konrad-Zuse-Zentrum fuer Informationstechnik Berlin (ZIB),O=GridGermany,C=DE",  // NOLINT
            user_mapping_->UIDToUsername(0));
  EXPECT_EQ(0,
            user_mapping_->UsernameToUID("CN=Patrick Schaefer,OU=CSR,OU=Konrad-Zuse-Zentrum fuer Informationstechnik Berlin (ZIB),O=GridGermany,C=DE"));  // NOLINT
...
TEST_F(UserMappingGridmapUnicoreTest, TestBasicDNAndOUResolving) {
  EXPECT_EQ("CN=Patrick Schaefer,OU=CSR,O=GridGermany,C=DE",
            user_mapping_->UIDToUsername(0));
  EXPECT_EQ(0,
            user_mapping_->UsernameToUID("CN=Patrick Schaefer,OU=CSR,O=GridGermany,C=DE"));  // NOLINT

  // All OUs will be returned as list of groups.
  list<string> list_of_groups;
  user_mapping_->GetGroupnames(0, 0, 0, &list_of_groups);
  ASSERT_EQ(1, list_of_groups.size());
  EXPECT_EQ("CSR",
            *(list_of_groups.begin()));
...
TEST_F(UserMappingGridmapUnicoreTest, GridmapFileReload) {
  EXPECT_EQ("CN=Patrick Schaefer,OU=CSR,O=GridGermany,C=DE",  // NOLINT
            user_mapping_->UIDToUsername(0));
  EXPECT_EQ(0,
            user_mapping_->UsernameToUID("CN=Patrick Schaefer,OU=CSR,O=GridGermany,C=DE"));  // NOLINT
