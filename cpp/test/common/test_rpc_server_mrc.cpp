...
namespace xtreemfs
{
  namespace rpc
  {
    ...
    TestRPCServerMRC::TestRPCServerMRC() : file_size_(1024 * 1024) {
      ...
      operations_[PROC_ID_XTREEMFS_RENEW_CAPABILITY] = Op(this,
                &TestRPCServerMRC::RenewCapabilityOperation);
      operations_[PROC_ID_XTREEMFS_UPDATE_FILE_SIZE] = Op(this,
          &TestRPCServerMRC::UpdateFileSizeOperation);
    }

    google::protobuf::Message*
    TestRPCServerMRC::OpenOperation(
    ...
    void
    TestRPCServerMRC::setFileSize(uint64_t size) {
      boost::mutex::scoped_lock lock(mutex_);
      file_size_ = size;
    }

    void
    TestRPCServerMRC::RegisterOSD(std::string uuid) {
      boost::mutex::scoped_lock lock(mutex_);
