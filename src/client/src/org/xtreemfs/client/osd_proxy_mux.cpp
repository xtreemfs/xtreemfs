...
OSDProxyMux::OSDProxyMux( YIELD::auto_Object<DIRProxy> dir_proxy, YIELD::auto_AIOQueue aio_queue, uint32_t flags, YIELD::auto_Log log, const YIELD::Time& operation_timeout, YIELD::auto_SSLContext ssl_context, YIELD::auto_StageGroup stage_group )
  : dir_proxy( dir_proxy ), aio_queue( aio_queue ), flags( flags ), log( log ), operation_timeout( operation_timeout ), ssl_context( ssl_context ), stage_group( stage_group )
{
  ...
  else
  {
    ...
    for ( org::xtreemfs::interfaces::AddressMappingSet::iterator address_mapping_i = address_mappings->begin(); address_mapping_i != address_mappings->end(); address_mapping_i++ )
    {
      ...
      if ( ssl_context != NULL && ( *address_mapping_i ).get_protocol() == org::xtreemfs::interfaces::ONCRPCS_SCHEME )
        tcp_osd_proxy = OSDProxy::create( ( *address_mapping_i ).get_uri(), osd_uuid, aio_queue, flags, log, operation_timeout, OSDProxy::PING_INTERVAL_DEFAULT, ssl_context ).release();
      else
#endif
...
void OSDProxyMux::handlereadRequest( readRequest& req )
{
  ...
  if ( req.get_response_target()->get_tag() != YIELD_OBJECT_TAG( ReadResponseTarget ) )
