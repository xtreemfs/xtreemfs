...
Proxy<ProxyType, InterfaceType>::Proxy( const YIELD::URI& absolute_uri, YIELD::auto_AIOQueue aio_queue, uint32_t flags, YIELD::auto_Log log, const YIELD::Time& operation_timeout, YIELD::auto_SocketAddress peer_sockaddr, YIELD::auto_SSLContext ssl_context )
  : YIELD::ONCRPCClient<InterfaceType>( absolute_uri, aio_queue, flags, log, operation_timeout, peer_sockaddr, ssl_context ), log( log )
{
#ifndef _WIN32
...
void Proxy<ProxyType, InterfaceType>::getpasswdFromUserCredentials( const std::string& user_id, const std::string& group_id, int& out_uid, int& out_gid )
{
  ...
  if ( !have_passwd )
  {
    ...
    else
    {
      ...
      out_gid = 0;
      if ( log != NULL )
        log->getStream( YIELD::Log::LOG_WARNING ) << "org::xtreemfs::client::Proxy: getpwnam_r and getgrnam_r with user_id=" << user_id << ", group_id=" << group_id << " failed, errno=" << errno << ", setting user/group to root.";
    }
  }
