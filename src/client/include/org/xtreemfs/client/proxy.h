...
namespace org
{
  namespace xtreemfs
  {
    namespace client
    {
      ...
      protected:
        Proxy( const YIELD::URI& absolute_uri, YIELD::auto_AIOQueue aio_queue, uint32_t flags, YIELD::auto_Log log, const YIELD::Time& operation_timeout, YIELD::auto_SocketAddress peer_sockaddr, YIELD::auto_SSLContext ssl_context );
        virtual ~Proxy();
