  ...
  namespace xtreemfs
  {
    namespace interfaces
    {
      ...
      const static uint32_t DEFAULT_HTTP_PORT = 30638;
        ...
        virtual void xtreemfs_address_mappings_remove( const std::string& uuid, uint64_t response_timeout_ns ) { ::YIELD::auto_Object<xtreemfs_address_mappings_removeRequest> __request( new xtreemfs_address_mappings_removeRequest( uuid ) ); ::YIELD::auto_OneSignalEventQueue __response_queue( new ::YIELD::OneSignalEventQueue ); __request->set_response_target( __response_queue->incRef() ); send( __request->incRef() ); ::YIELD::auto_Object<xtreemfs_address_mappings_removeResponse> __response = __response_queue->dequeue_typed<xtreemfs_address_mappings_removeResponse>( response_timeout_ns ); }
