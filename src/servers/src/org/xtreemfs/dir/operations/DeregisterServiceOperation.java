...
public class DeregisterServiceOperation extends DIROperation {
    ...
    public void startRequest(DIRRequest rq) {
        xtreemfs_service_deregisterRequest request = 
            (xtreemfs_service_deregisterRequest)rq.getRequestMessage();

        BabuDBInsertGroup ig;
        try {  
            ig = database.createInsertGroup();
            ig.addDelete(DIRRequestDispatcher.INDEX_ID_SERVREG, request.getUuid()
                    .getBytes());
    ...
    void requestFinished(Object result, DIRRequest rq) {
        rq.sendSuccess(new xtreemfs_service_deregisterResponse());
    }
