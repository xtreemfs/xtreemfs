...
public class UserCredentials extends Struct
{
    ...
    public int getXDRSize()
    {
        ...
        my_size += password != null ? ( ( password.getBytes().length + Integer.SIZE/8 ) % 4 == 0 ) ? ( password.getBytes().length + Integer.SIZE/8 ) : ( password.getBytes().length + Integer.SIZE/8 + 4 - ( password.getBytes().length + Integer.SIZE/8 ) % 4 ) : 0;
        return my_size;
    }    
