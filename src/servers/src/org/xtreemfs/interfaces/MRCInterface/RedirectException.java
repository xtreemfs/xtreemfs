...
public class RedirectException extends org.xtreemfs.interfaces.utils.ONCRPCException
{
    ...
    public static final long serialVersionUID = 2009082873;    
    ...
    {
        int my_size = 0;
        my_size += ( ( to_uuid.getBytes().length + Integer.SIZE/8 ) % 4 == 0 ) ? ( to_uuid.getBytes().length + Integer.SIZE/8 ) : ( to_uuid.getBytes().length + Integer.SIZE/8 + 4 - ( to_uuid.getBytes().length + Integer.SIZE/8 ) % 4 );
        return my_size;
    }    
