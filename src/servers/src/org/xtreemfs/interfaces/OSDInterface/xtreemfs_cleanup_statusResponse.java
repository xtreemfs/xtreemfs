...
public class xtreemfs_cleanup_statusResponse extends org.xtreemfs.interfaces.utils.Response
{
    ...
    {
        int my_size = 0;
        my_size += ( ( status.getBytes().length + Integer.SIZE/8 ) % 4 == 0 ) ? ( status.getBytes().length + Integer.SIZE/8 ) : ( status.getBytes().length + Integer.SIZE/8 + 4 - ( status.getBytes().length + Integer.SIZE/8 ) % 4 );
        return my_size;
    }    
