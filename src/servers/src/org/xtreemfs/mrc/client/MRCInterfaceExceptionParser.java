 ...
 * @author bjko
 */
public class MRCInterfaceExceptionParser extends RemoteExceptionParser {

    public MRCInterfaceExceptionParser() {

    }

    @Override
    public boolean canParseException(int accept_stat) {
        return (accept_stat >= MRCInterface.getVersion() && (accept_stat < MRCInterface.getVersion()+100));
    }

    @Override
    public ONCRPCException parseException(int accept_stat, XDRUnmarshaller unmarshaller) throws IOException {
        try {
            ONCRPCException ex = MRCInterface.createException(accept_stat);
