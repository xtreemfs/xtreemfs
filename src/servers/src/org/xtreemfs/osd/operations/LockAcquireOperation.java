...
public class LockAcquireOperation extends OSDOperation {
    ...
    public LockAcquireOperation(OSDRequestDispatcher master) {
        super(master);
        procId = xtreemfs_lock_acquireRequest.TAG;
...
//        System.out.println("rq: " + args);
        ...
        master.getPreprocStage().acquireLock(args.getClient_uuid(), args.getClient_pid(), args.getFile_id(),
    ...
    public void postAcquireLock(final OSDRequest rq, xtreemfs_lock_acquireRequest args,
        ...
        if (error != null) {
            ...
            xtreemfs_lock_acquireResponse resp = new xtreemfs_lock_acquireResponse(lock);
            rq.sendSuccess(resp);
        }
    }
    ...
    public Serializable parseRPCMessage(ReusableBuffer data, OSDRequest rq) throws Exception {
        xtreemfs_lock_acquireRequest rpcrq = new xtreemfs_lock_acquireRequest();
