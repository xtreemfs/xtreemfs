 ...
 * 01.04.2009
 */
public class EventGetFileSize extends OSDOperation {

    public EventGetFileSize(OSDRequestDispatcher master) {
        super(master);
    }
    ...
    public void startInternalEvent(Object[] args) {
        ...
        final Long updateObjVer = (Long) args[1];
        final StripingPolicyImpl sp = (StripingPolicyImpl) args[2];

        master.getStorageStage().internalGetMaxObjectNo(fileId, sp, new InternalGetMaxObjectNoCallback() {

            @Override
            public void maxObjectNoCompleted(long maxObjNo, long fileSize, long truncateEpoch, Exception error) {
                master.getRWReplicationStage().eventFileSizeAvailable(fileId, fileSize, truncateEpoch, updateObjVer, error);
            }
        });
    }
