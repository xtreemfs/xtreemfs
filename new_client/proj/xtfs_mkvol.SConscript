...
for custom_SConscript in ["xtfs_mkvol_custom.SConscript"]:
    if FindFile( custom_SConscript, "." ):
        SConscript( custom_SConscript )

    
# Don't add libs until after xtfs_mkvol_custom.SConscript and dependency SConscripts, to avoid failing build_conf checks because of missing -l libs
for lib in []:
...
AlwaysBuild( build_env.Program( r"../bin/xtfs_mkvol", (
r"../src/org/xtreemfs/client/bin/xtfs_mkvol.cpp"
) ) )
