...
# google-protobuf always tries to run its unit tests with a given 'protoc'. Make sure that these tests always pass by setting the 'protoc' path to '/bin/true'.
	...
	(cd $(@D) && CC="$(TARGET_CC)" CXX="$(TARGET_CXX)" LD="$(TARGET_LD) -L$(STAGING_DIR)/lib -L$(STAGING_DIR)/usr/lib" cmake -DSKIP_FUSE=true -Hcpp -B$(@D)/cpp/build --check-build-system CMakeFiles/Makefile.cmake 0  -DBOOST_ROOT=$(STAGING_DIR)/usr)
